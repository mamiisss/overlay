...
-- then mounts the bridge UI. This file is generated by scripts/build-bundles.js.

local EMBEDDED_RUNTIME_CLIENT = [=[local CONFIG={url='ws://127.0.0.1:8080/realtime',token=nil,room_name=
'Overlay Dev Room',client_name='overlay-runtime',build='0.1.0',move_hz=10,
move_keepalive_seconds=10,interp_delay=0.25,request_timeout=5,
reconnect_max_delay=15,follow_room_route=true,auto_join_default_room=false,
max_native_highlights=25,asset_cache_folder='overlay-cache/assets',
asset_max_bytes=32*1024*1024,asset_require_hash_verification=false,
asset_catalog_url=nil,native_morph_archive_path=
...
pcall(function()CONFIG.collection_service=game:GetService('CollectionService')
end)local AssetService=nil pcall(function()AssetService=game:GetService(
'AssetService')end)local LocalPlayer=Players.LocalPlayer local GLOBAL=typeof(
getgenv)=='function'and getgenv()or _G if type(GLOBAL.OverlayAssetCatalogUrl)==
'string'then CONFIG.asset_catalog_url=GLOBAL.OverlayAssetCatalogUrl end if type(
GLOBAL.OverlayNativeMorphArchivePath)=='string'then CONFIG.
native_morph_archive_path=GLOBAL.OverlayNativeMorphArchivePath end if type(
GLOBAL.OverlayNativeMorphArchiveUrl)=='string'then CONFIG.
native_morph_archive_url=GLOBAL.OverlayNativeMorphArchiveUrl end if type(GLOBAL.
OverlayNativeAnimationArchivePath)=='string'then CONFIG.
native_animation_archive_path=GLOBAL.OverlayNativeAnimationArchivePath end if
type(GLOBAL.OverlayNativeAnimationArchiveUrl)=='string'then CONFIG.
...
false elseif GLOBAL.OverlayNativeMorphSmartBones==true then CONFIG.
native_morph_enable_smart_bones=true end if GLOBAL.
OverlayNativeMorphInferSmartBoneRoots==false then CONFIG.
native_morph_infer_smart_bone_roots=false elseif GLOBAL.
OverlayNativeMorphInferSmartBoneRoots==true then CONFIG.
native_morph_infer_smart_bone_roots=true end if tonumber(GLOBAL.
OverlayNativeMorphMaxSmartBoneControllers)then CONFIG.
native_morph_max_smart_bone_controllers=math.max(0,math.floor(tonumber(GLOBAL.
OverlayNativeMorphMaxSmartBoneControllers)))end if tonumber(GLOBAL.
OverlayNativeMorphMaxSmartBoneParticles)then CONFIG.
native_morph_max_smart_bone_particles=math.max(0,math.floor(tonumber(GLOBAL.
OverlayNativeMorphMaxSmartBoneParticles)))end if tonumber(GLOBAL.
OverlayNativeMorphSmartBoneFrameBudgetSeconds)then CONFIG.
native_morph_smart_bone_frame_budget_seconds=math.max(0,tonumber(GLOBAL.
OverlayNativeMorphSmartBoneFrameBudgetSeconds))end local function log(...)print(
'[overlay]',...)end if type(GLOBAL.OverlayStop)=='function'then pcall(GLOBAL.
OverlayStop)end local function wsConnect(url)local candidates={}local function 
safeField(owner,key)if owner==nil then return nil end local ok,value=pcall(
function()return owner[key]end)if ok then return value end return nil end local 
function globalValue(name)local sources={GLOBAL,_G}if type(shared)=='table'then
table.insert(sources,shared)end if type(getrenv)=='function'then local ok,env=
pcall(getrenv)if ok and type(env)=='table'then table.insert(sources,env)end end
if type(getfenv)=='function'then for _,level in ipairs({0,1,2,3})do local ok,env
=pcall(getfenv,level)if ok and type(env)=='table'then table.insert(sources,env)
end end end for _,source in ipairs(sources)do local value=safeField(source,name)
if value~=nil then return value end end return nil end local function 
addCandidate(label,owner,method)if type(method)=='function'then table.insert(
candidates,{label=label,connect=function(targetUrl)return method(owner,targetUrl
)end})table.insert(candidates,{label=label..' direct',connect=function(targetUrl
)return method(targetUrl)end})end end for _,name in ipairs({'WebSocket',
'Websocket','websocket'})do local api=globalValue(name)if api~=nil then
addCandidate(name..'.connect',api,safeField(api,'connect'))addCandidate(name..
'.Connect',api,safeField(api,'Connect'))addCandidate(name..'.CONNECT',api,
safeField(api,'CONNECT'))end end local synApi=globalValue('syn')local
synWebsocket=safeField(synApi,'websocket')if synWebsocket~=nil then
addCandidate('syn.websocket.connect',synWebsocket,safeField(synWebsocket,
'connect'))addCandidate('syn.websocket.Connect',synWebsocket,safeField(
synWebsocket,'Connect'))end addCandidate('websocket_connect',nil,globalValue(
'websocket_connect'))addCandidate('WebSocketConnect',nil,globalValue(
'WebSocketConnect'))local function callSocketMethod(method,raw,...)local ok,
result=pcall(method,raw,...)if ok then return result end return method(...)end
local function normalizeSocket(raw)if raw==nil then return nil end local
sendMethod=safeField(raw,'Send')or safeField(raw,'send')local closeMethod=
safeField(raw,'Close')or safeField(raw,'close')local onMessage=safeField(raw,
'OnMessage')or safeField(raw,'MessageReceived')or safeField(raw,'Message')local
onClose=safeField(raw,'OnClose')or safeField(raw,'Closed')or safeField(raw,
'CloseEvent')if type(sendMethod)~='function'or type(closeMethod)~='function'then
return nil end if onMessage==nil or onClose==nil then return nil end if type(
safeField(onMessage,'Connect'))~='function'or type(safeField(onClose,'Connect'))
~='function'then return nil end return{raw=raw,OnMessage=onMessage,OnClose=
onClose,Send=function(_,message)return callSocketMethod(sendMethod,raw,message)
end,Close=function()return callSocketMethod(closeMethod,raw)end}end for _,
candidate in ipairs(candidates)do local ok,ws=pcall(candidate.connect,url)if ok
then local normalized=normalizeSocket(ws)if normalized then return normalized
end end end return nil end local Client={running=true,ws=nil,connected=false,
nextId=0,pending={},user=nil,room=nil,joining=nil,deferredSnapshot=nil,
ownAvatarId=nil,avatarReadyRoomId=nil,entities={},connections={},wsConnections={
},sessionId=0,heartbeatSeconds=15,lastServerContact=0,teleporting=false,
roomClosingId=nil,bridgeSubscribers={},cachedRooms={},assetManifests={},
assetCache={},assetRequests={},assetDescriptors={},assetCatalog={},
assetCatalogById={},nativeMorphArchive=nil,nativeMorphArchiveStatus='idle',
nativeMorphArchiveError=nil,nativeMorphArchiveLoading=false,nativeMorphCatalog={
},nativeMorphCatalogIndex={},nativeMorphCatalogSlots={},nativeMorphLookup={},
nativeMorphStats={},nativeMorphLoadAttempted=false,nativeMorphCatalogEmitted=
false,nativeAnimationArchive=nil,nativeAnimationArchiveStatus='idle',
...
abs(ry),math.abs(rz))return math.deg(maxAngle)<1 end local function 
nextMessageId()Client.nextId=Client.nextId+1 return'msg_'..Client.nextId end
local function sendMessageWithId(id,messageType,data)if not Client.connected or
Client.ws==nil then return false end local ok,encoded=pcall(HttpService.
JSONEncode,HttpService,{t=messageType,id=id,v=1,data=data or{}})if not ok then
return false end local sent=pcall(function()Client.ws:Send(encoded)end)if not
sent then Client.connected=false return false end return true end local function 
sendMessage(messageType,data)local id=nextMessageId()if not sendMessageWithId(id
,messageType,data)then return nil end return id end local function awaitRequest(
messageType,data,timeoutSeconds)local id=nextMessageId()local result Client.
pending[id]={callback=function(message)result=message end,expiresAt=os.clock()+(
timeoutSeconds or CONFIG.request_timeout)}if not sendMessageWithId(id,
messageType,data)then Client.pending[id]=nil return nil,'not connected'end local
deadline=os.clock()+(timeoutSeconds or CONFIG.request_timeout)while result==nil
and os.clock()<deadline and Client.running and Client.connected do task.wait(
0.05)end Client.pending[id]=nil if result==nil then return nil,
'timeout waiting for '..messageType end if result.t=='error'then return nil,
tostring(result.data and result.data.code)..': '..tostring(result.data and
result.data.message)end return result end local function 
currentRobloxCreateRoute()return{roblox_place_id=game.PlaceId,roblox_job_id=
tostring(game.JobId or''),roblox_universe_id=game.GameId}end local function 
currentRobloxJoinContext()return{client_roblox_place_id=game.PlaceId,
client_roblox_job_id=tostring(game.JobId or''),client_roblox_universe_id=game.
GameId}end local function mergeTables(left,right)local merged={}for key,value in
pairs(left or{})do merged[key]=value end for key,value in pairs(right or{})do
merged[key]=value end return merged end local function sameRobloxRoute(route)if
type(route)~='table'or tonumber(route.place_id)==nil then return true end if
tonumber(route.place_id)~=game.PlaceId then return false end local jobId=type(
route.job_id)=='string'and route.job_id or''if jobId~=''and jobId~=tostring(game
.JobId or'')then return false end return true end local function 
tryTeleportToRoute(route,roomId)if not CONFIG.follow_room_route then return
false,'room route following disabled'end if type(route)~='table'or tonumber(
route.place_id)==nil then return false,'missing roblox route'end if
sameRobloxRoute(route)then return false,'already in target Roblox server'end
pcall(function()TeleportService:SetTeleportSetting('overlay_room_id',roomId)end)
local placeId=tonumber(route.place_id)local jobId=type(route.job_id)=='string'
and route.job_id or''Client.teleporting=true log('teleporting to room server',
'place',placeId,'job',jobId~=''and jobId or'<any>')local ok,err=pcall(function()
if jobId~=''then TeleportService:TeleportToPlaceInstance(placeId,jobId,
LocalPlayer)else TeleportService:Teleport(placeId,LocalPlayer)end end)if ok then
return true,nil end Client.teleporting=false return false,tostring(err)end local 
function pendingRoomFromTeleportSetting()local ok,roomId=pcall(function()return
TeleportService:GetTeleportSetting('overlay_room_id')end)if ok and type(roomId)
=='string'and roomId~=''then pcall(function()TeleportService:SetTeleportSetting(
'overlay_room_id',nil)end)return roomId end return nil end local function 
trimString(value)if value==nil then return''end return tostring(value):match(
'^%s*(.-)%s*$')end local function yieldIfBudgetSpent(startedAt)local budget=
tonumber(CONFIG.native_morph_operation_budget_seconds)or 0 if budget>0 and os.
clock()-startedAt>=budget then task.wait()return os.clock()end return startedAt
end local function emitBridgeEvent(eventName,payload)for _,callback in ipairs(
Client.bridgeSubscribers)do task.spawn(function()pcall(callback,eventName,
payload or{})end)end end local function emitBridgeState(state)emitBridgeEvent(
'diagnostics.state',{state=state})end local function emitBridgeError(code,
message)emitBridgeEvent('error',{code=code or'runtime.error',message=message or
code or'Runtime error'})end local AssetCache={}local function getGlobalFunction(
name)if type(GLOBAL)=='table'and type(GLOBAL[name])=='function'then return
GLOBAL[name]end if type(_G)=='table'and type(_G[name])=='function'then return _G
[name]end return nil end local function firstCallable(...)local values={...}for
_,value in ipairs(values)do if type(value)=='function'then return value end end
return nil end local function requestHttp(url)local candidates={
getGlobalFunction('request'),getGlobalFunction('http_request'),typeof(syn)==
'table'and syn.request or nil,typeof(http)=='table'and http.request or nil}for _
,requestFn in ipairs(candidates)do if type(requestFn)=='function'then local ok,
response=pcall(requestFn,{Url=url,Method='GET'})if ok and response then return
response end ok,response=pcall(requestFn,url)if ok and response then return
response end end end local ok,body=pcall(function()return game:HttpGet(url)end)
if ok and type(body)=='string'then return body end return nil end local function 
responseBody(response)if type(response)=='string'then return response end if
type(response)~='table'then return nil end if response.Success==false then
return nil end local status=tonumber(response.StatusCode or response.status_code
or response.status)if status and status>=400 then return nil end return response
.Body or response.body or response.Data or response.data end local function 
ensureFolder(path)local makefolderFn=getGlobalFunction('makefolder')local
isfolderFn=getGlobalFunction('isfolder')if not makefolderFn or not isfolderFn
then return false end local current=''for part in string.gmatch(path,'[^/]+')do
current=current==''and part or(current..'/'..part)if not isfolderFn(current)then
local ok=pcall(makefolderFn,current)if not ok and not isfolderFn(current)then
return false end end end return true end local function safeFileName(value)value
=tostring(value or'asset')value=value:gsub('[^%w%._%-]','_')return string.sub(
value,1,96)end local function parentFolder(path)path=tostring(path or'')return
string.match(path,'^(.*)/[^/]+$')or''end local function assetExtension(asset)
local url=tostring(asset and asset.url or'')local path=string.match(url,
'^[^%?]+')or url local ext=string.match(path,'%.([%w]+)$')if ext and#ext<=8 then
return'.'..string.lower(ext)end local assetType=string.lower(tostring(asset and
asset.type or''))if string.find(assetType,'texture',1,true)or string.find(
assetType,'image',1,true)then return'.png'end if string.find(assetType,'mesh',1,
true)then return'.mesh'end return'.bin'end local function assetCachePath(asset)
local key=safeFileName((asset.asset_id or'asset')..'_'..(asset.hash or'nohash'))
return CONFIG.asset_cache_folder..'/'..key..assetExtension(asset)end local 
function calculateSha256(body)local cryptTable=typeof(crypt)=='table'and crypt
or nil local synCryptTable=typeof(syn)=='table'and typeof(syn.crypt)=='table'and
syn.crypt or nil local hashFn=firstCallable(cryptTable and cryptTable.hash,
synCryptTable and synCryptTable.hash,getGlobalFunction('sha256'))if not hashFn
then return nil end local attempts={function()return hashFn(body,'sha256')end,
function()return hashFn('sha256',body)end,function()return hashFn(body)end}for _
,attempt in ipairs(attempts)do local ok,value=pcall(attempt)if ok and type(value
)=='string'and value~=''then return string.lower(value)end end return nil end
local function verifyAssetBody(asset,body)local expected=string.lower(tostring(
asset.hash or''))if string.sub(expected,1,7)=='sha256:'then local actual=
calculateSha256(body)if actual then return actual==string.sub(expected,8),actual
==string.sub(expected,8)and'sha256'or'sha256_mismatch'end return not CONFIG.
asset_require_hash_verification,'sha256_unavailable'end if string.sub(expected,1
,4)=='dev:'or string.sub(expected,1,7)=='unsafe:'then return not CONFIG.
asset_require_hash_verification,'dev_hash'end return not CONFIG.
asset_require_hash_verification,'unsupported_hash'end function AssetCache.
localUrl(assetOrId)local assetId=type(assetOrId)=='table'and assetOrId.asset_id
or assetOrId if not assetId then return nil end local cached=Client.assetCache[
assetId]if cached and cached.localUrl then return cached.localUrl end local
asset=type(assetOrId)=='table'and assetOrId or Client.assetManifests[assetId]if
not asset then return nil end local path=assetCachePath(asset)local isfileFn=
getGlobalFunction('isfile')local getcustomassetFn=getGlobalFunction(
'getcustomasset')if not isfileFn or not getcustomassetFn or not isfileFn(path)
then return nil end local ok,localUrl=pcall(getcustomassetFn,path)if not ok or
not localUrl then return nil end Client.assetCache[assetId]={status='ready',path
=path,localUrl=localUrl,verified=cached and cached.verified or false}return
localUrl end function AssetCache.localPath(assetOrId)local assetId=type(
assetOrId)=='table'and assetOrId.asset_id or assetOrId if not assetId then
return nil end local cached=Client.assetCache[assetId]if cached and cached.path
then return cached.path end local asset=type(assetOrId)=='table'and assetOrId or
Client.assetManifests[assetId]if not asset then return nil end local path=
assetCachePath(asset)local isfileFn=getGlobalFunction('isfile')if isfileFn and
isfileFn(path)then return path end return nil end function AssetCache.body(
assetOrId)local path=AssetCache.localPath(assetOrId)local readfileFn=
getGlobalFunction('readfile')if not path or not readfileFn then return nil end
local ok,body=pcall(readfileFn,path)if ok and type(body)=='string'then return
body end return nil end function AssetCache.requestManifest(assetId)assetId=
tostring(assetId or'')if assetId==''or Client.assetManifests[assetId]or Client.
assetRequests['manifest:'..assetId]then return end Client.assetRequests[
'manifest:'..assetId]=true task.spawn(function()local response=nil if Client.
connected then response=awaitRequest('asset.get',{asset_id=assetId},CONFIG.
request_timeout)end Client.assetRequests['manifest:'..assetId]=nil local asset=
response and response.data and response.data.asset if asset then AssetCache.
registerAssets({asset})end end)end function AssetCache.ensure(asset)if type(
asset)~='table'or type(asset.asset_id)~='string'or type(asset.url)~='string'then
return false end Client.assetManifests[asset.asset_id]=asset if AssetCache.
localUrl(asset)then return true end if Client.assetRequests['download:'..asset.
asset_id]then return false end local writefileFn=getGlobalFunction('writefile')
if not writefileFn or not getGlobalFunction('isfile')or not getGlobalFunction(
'getcustomasset')then Client.assetCache[asset.asset_id]={status=
'missing_filesystem'}return false end if asset.size and tonumber(asset.size)and
tonumber(asset.size)>CONFIG.asset_max_bytes then Client.assetCache[asset.
asset_id]={status='too_large'}return false end Client.assetRequests['download:'
..asset.asset_id]=true task.spawn(function()local body=responseBody(requestHttp(
asset.url))if type(body)~='string'then Client.assetCache[asset.asset_id]={status
='download_failed'}Client.assetRequests['download:'..asset.asset_id]=nil
emitBridgeEvent('asset.cached',{asset_id=asset.asset_id,status='download_failed'
})return end if#body>CONFIG.asset_max_bytes then Client.assetCache[asset.
asset_id]={status='too_large'}Client.assetRequests['download:'..asset.asset_id]=
nil emitBridgeEvent('asset.cached',{asset_id=asset.asset_id,status='too_large'})
return end local verified,verification=verifyAssetBody(asset,body)if not
verified and CONFIG.asset_require_hash_verification then Client.assetCache[asset
.asset_id]={status='verify_failed',verification=verification}Client.
assetRequests['download:'..asset.asset_id]=nil emitBridgeEvent('asset.cached',{
asset_id=asset.asset_id,status='verify_failed',verification=verification})return
end if not ensureFolder(CONFIG.asset_cache_folder)then Client.assetCache[asset.
asset_id]={status='cache_folder_failed'}Client.assetRequests['download:'..asset.
asset_id]=nil emitBridgeEvent('asset.cached',{asset_id=asset.asset_id,status=
'cache_folder_failed'})return end local path=assetCachePath(asset)local ok=
pcall(writefileFn,path,body)if not ok then Client.assetCache[asset.asset_id]={
status='write_failed'}Client.assetRequests['download:'..asset.asset_id]=nil
emitBridgeEvent('asset.cached',{asset_id=asset.asset_id,status='write_failed'})
return end local localUrl=AssetCache.localUrl(asset)Client.assetCache[asset.
asset_id]={status=localUrl and'ready'or'custom_asset_failed',path=path,localUrl=
localUrl,verified=verified,verification=verification}Client.assetRequests[
'download:'..asset.asset_id]=nil emitBridgeEvent('asset.cached',{asset_id=asset.
asset_id,status=Client.assetCache[asset.asset_id].status,verified=verified,
verification=verification})if refreshRenderers then refreshRenderers()end end)
return false end function AssetCache.registerAssets(assets)for _,asset in
ipairs(assets or{})do if type(asset)=='table'and type(asset.asset_id)=='string'
then Client.assetManifests[asset.asset_id]=asset AssetCache.ensure(asset)for _,
dependencyId in ipairs(asset.dependencies or{})do AssetCache.requestManifest(
dependencyId)end end end end local function registerCatalogAssets(assets)Client.
assetCatalog={}Client.assetCatalogById={}for _,asset in ipairs(assets or{})do if
type(asset)=='table'and type(asset.asset_id)=='string'then table.insert(Client.
assetCatalog,asset)Client.assetCatalogById[asset.asset_id]=asset end end
emitBridgeEvent('asset.catalog',{assets=Client.assetCatalog,total=#Client.
assetCatalog})end local function loadAssetCatalog(url)url=trimString(url)if url
==''then emitBridgeError('asset.catalog_url_required','Catalog URL is required')
return end task.spawn(function()local body=responseBody(requestHttp(url))if
type(body)~='string'then emitBridgeError('asset.catalog_failed',
'Could not fetch asset catalog')return end local ok,catalog=pcall(HttpService.
JSONDecode,HttpService,body)if not ok or type(catalog)~='table'or type(catalog.
assets)~='table'then emitBridgeError('asset.catalog_invalid',
'Invalid asset catalog')return end local function catalogHttpUrl(value)value=
string.lower(trimString(value))return string.sub(value,1,7)=='http://'or string.
sub(value,1,8)=='https://'end local baseUrl=trimString(catalog.base_url)if
baseUrl==''then baseUrl=string.match(url,'^(.*)/[^/]*$')or url end for _,asset
in ipairs(catalog.assets)do if type(asset)=='table'and type(asset.url)=='string'
and not catalogHttpUrl(asset.url)then asset.url=baseUrl:gsub('/+$','')..'/'..
asset.url:gsub('^/+','')end end registerCatalogAssets(catalog.assets)
emitBridgeState('asset catalog loaded: '..tostring(#Client.assetCatalog))end)end
local function collectAssetIdsFromValue(value,out,depth)if type(value)~='table'
or depth>8 then return end if type(value.asset_id)=='string'then out[value.
asset_id]=true end for _,child in pairs(value)do if type(child)=='table'then
collectAssetIdsFromValue(child,out,depth+1)end end end local function 
prefetchAssetRefs(components)local ids={}collectAssetIdsFromValue(components,ids
,0)for assetId in pairs(ids)do local asset=Client.assetManifests[assetId]if
asset then AssetCache.ensure(asset)else AssetCache.requestManifest(assetId)end
end end local function isOverlayDescriptorAsset(asset)if type(asset)~='table'
then return false end local format=string.lower(tostring(asset.format or''))
local typeName=string.lower(tostring(asset.type or asset.kind or''))local url=
string.lower(tostring(asset.url or''))return format=='overlay_json_v1'or string.
find(typeName,'morph_bundle',1,true)~=nil or string.find(url,'%.json',1,false)~=
nil end local function descriptorForAsset(assetId)assetId=trimString(assetId)if
assetId==''then return nil end if Client.assetDescriptors[assetId]then return
Client.assetDescriptors[assetId]end local asset=Client.assetManifests[assetId]if
not asset then AssetCache.requestManifest(assetId)return nil end if not
isOverlayDescriptorAsset(asset)then return nil end local body=AssetCache.body(
asset)if type(body)~='string'then AssetCache.ensure(asset)return nil end local
ok,descriptor=pcall(HttpService.JSONDecode,HttpService,body)if not ok or type(
descriptor)~='table'or descriptor.format~='overlay_json_v1'or type(descriptor.
nodes)~='table'then Client.assetDescriptors[assetId]=false emitBridgeError(
'asset.descriptor_invalid','Invalid overlay descriptor: '..tostring(assetId))
return nil end Client.assetDescriptors[assetId]=descriptor emitBridgeEvent(
'asset.descriptor.ready',{asset_id=assetId,name=descriptor.name,nodes=#
descriptor.nodes})return descriptor end local function colorFromId(entityId)
local hash=0 for i=1,#entityId do hash=(hash*31+string.byte(entityId,i))%
16777216 end return Color3.fromHSV((hash%360)/360,0.6,0.9)end local function 
colorToArray(color)return{color.R,color.G,color.B}end local function 
colorFromComponents(entityId,components)local avatar=components and components.
avatar local custom=avatar and avatar.overlay_color if type(custom)=='table'then
local r=tonumber(custom[1])local g=tonumber(custom[2])local b=tonumber(custom[3]
)if r and g and b then return Color3.new(math.clamp(r,0,1),math.clamp(g,0,1),
math.clamp(b,0,1))end end if type(avatar)=='table'and tonumber(avatar.
appearance_user_id)then return colorFromId(tostring(avatar.appearance_user_id))
end return colorFromId(entityId)end local EFFECT_PRESETS={none={color=Color3.
fromRGB(170,170,170)},violet={color=Color3.fromRGB(155,105,255)},cyan={color=
Color3.fromRGB(80,220,255)},emerald={color=Color3.fromRGB(80,255,170)},gold={
color=Color3.fromRGB(255,205,80)},rose={color=Color3.fromRGB(255,95,150)}}local
NativeAnimation={}local function colorFromEffect(effect,fallback)if type(effect)
=='table'and type(effect.color)=='table'then local r=tonumber(effect.color[1])
local g=tonumber(effect.color[2])local b=tonumber(effect.color[3])if r and g and
b then return Color3.new(math.clamp(r,0,1),math.clamp(g,0,1),math.clamp(b,0,1))
end end local presetName='violet'if type(effect)=='table'and type(effect.preset)
=='string'then presetName=string.lower(effect.preset)end local preset=
EFFECT_PRESETS[presetName]return(preset and preset.color)or fallback end local 
function effectSettings(entityId,components)local effect=type(components)==
'table'and components.effect or nil local fallbackColor=colorFromComponents(
entityId,components)local intensity=35 local preset='violet'local trail=false if
type(effect)=='table'then intensity=tonumber(effect.intensity)or tonumber(effect
.density)or intensity if type(effect.preset)=='string'then preset=string.lower(
effect.preset)end trail=effect.trail==true or effect.trail=='on'or effect.trail
=='true'end intensity=math.clamp(intensity,0,100)if preset=='none'then intensity
=0 end return{enabled=intensity>0,intensity=intensity,alpha=intensity/100,color=
colorFromEffect(effect,fallbackColor),trail=trail}end local function 
transformToCFrame(transform)local p=transform.position local r=transform.
rotation local position=Vector3.new(tonumber(p and p[1])or 0,tonumber(p and p[2]
)or 5,tonumber(p and p[3])or 0)local rx=math.rad(tonumber(r and r[1])or 0)local
ry=math.rad(tonumber(r and r[2])or 0)local rz=math.rad(tonumber(r and r[3])or 0)
return CFrame.new(position)*CFrame.fromOrientation(rx,ry,rz)end local function 
displayNameFor(entityId,components)local avatar=components and components.avatar
if type(avatar)=='table'and type(avatar.display_name)=='string'then return
avatar.display_name end return entityId end local function robloxUserIdFor(
components)local avatar=components and components.avatar if type(avatar)~=
'table'then return nil end local id=tonumber(avatar.roblox_user_id)if id==nil
then return nil end return id end local function findPlayerByRobloxUserId(
robloxUserId)if robloxUserId==nil then return nil end for _,player in ipairs(
Players:GetPlayers())do if player.UserId==robloxUserId then return player end
end return nil end local function isOwnEntity(entityId,components)if entityId==
Client.ownAvatarId then return true end return robloxUserIdFor(components)==
LocalPlayer.UserId end local function createBillboard(parent,entityId,components
,offset)local color=colorFromComponents(entityId,components)local billboard=
Instance.new('BillboardGui')billboard.Name='OverlayName'billboard.Size=UDim2.
fromOffset(180,30)billboard.StudsOffset=offset or Vector3.new(0,2.6,0)billboard.
AlwaysOnTop=true billboard.Parent=parent local label=Instance.new('TextLabel')
label.Name='Label'label.Size=UDim2.fromScale(1,1)label.BackgroundTransparency=1
label.TextScaled=true label.Font=Enum.Font.GothamBold label.TextColor3=color
...
value==''then return nil end local lower=string.lower(value)if string.sub(lower,
1,13)=='rbxassetid://'then return value end local digits=string.match(value,
'^%s*(%d+)%s*$')if digits then return'rbxassetid://'..digits end return nil end
local function animationAssetUrl(animation)if type(animation)~='table'then
return nil end for _,key in ipairs({'animation_id','animationId',
'roblox_asset_id','robloxAssetId','url','asset_url','assetUrl','asset_id','id'})
do local value=animation[key]if type(value)=='number'or type(value)=='string'
then local url=robloxAssetUrl(value)if url then return url end end end local
assetId=assetIdFromComponent(animation)local manifest=assetId and Client.
assetManifests[assetId]or nil if type(manifest)=='table'then for _,key in
ipairs({'animation_id','animationId','roblox_asset_id','robloxAssetId','url',
'asset_url','assetUrl'})do local url=robloxAssetUrl(manifest[key])if url then
return url end end end return nil end local function clearAnimation(handle)if
handle==nil then return end if NativeAnimation and type(NativeAnimation.clear)==
'function'then NativeAnimation.clear(handle)end if handle.animationTrack then
pcall(function()handle.animationTrack:Stop(0.15)handle.animationTrack:Destroy()
end)handle.animationTrack=nil end if handle.animationObject then pcall(function(
)handle.animationObject:Destroy()end)handle.animationObject=nil end handle.
animationKey=nil handle.animationCharacter=nil end local function 
...
end end local function descriptorVector3(value,fallback)if type(value)~='table'
then return fallback or Vector3.new()end return Vector3.new(tonumber(value[1])or
0,tonumber(value[2])or 0,tonumber(value[3])or 0)end local function 
descriptorVector2(value,fallback)if type(value)~='table'then return fallback or
Vector2.new()end return Vector2.new(tonumber(value[1])or 0,tonumber(value[2])or
0)end local function descriptorColor(value,fallback)if type(value)~='table'then
return fallback or Color3.new(1,1,1)end return Color3.new(math.clamp(tonumber(
value[1])or 1,0,1),math.clamp(tonumber(value[2])or 1,0,1),math.clamp(tonumber(
value[3])or 1,0,1))end local function descriptorCFrame(value)if type(value)~=
'table'then return CFrame.new()end local position=value.position or{}local
rotation=value.rotation or{}return CFrame.new(tonumber(position[1])or 0,
tonumber(position[2])or 0,tonumber(position[3])or 0,tonumber(rotation[1])or 1,
tonumber(rotation[2])or 0,tonumber(rotation[3])or 0,tonumber(rotation[4])or 0,
tonumber(rotation[5])or 1,tonumber(rotation[6])or 0,tonumber(rotation[7])or 0,
tonumber(rotation[8])or 0,tonumber(rotation[9])or 1)end local function 
descriptorNumberRange(value,fallbackMin,fallbackMax)if type(value)=='table'then
local minValue=tonumber(value[1])or fallbackMin or 0 local maxValue=tonumber(
value[2])or minValue return NumberRange.new(minValue,maxValue)end local numeric=
tonumber(value)if numeric then return NumberRange.new(numeric)end return
NumberRange.new(fallbackMin or 0,fallbackMax or fallbackMin or 0)end local 
function parseDescriptorNumbers(value)local numbers={}for part in string.gmatch(
tostring(value or''),'%S+')do local numeric=tonumber(part)if numeric then table.
insert(numbers,numeric)end end return numbers end local function 
descriptorNumberSequence(value,fallback)if type(value)~='string'or value==''then
return NumberSequence.new(fallback or 0)end local numbers=
parseDescriptorNumbers(value)local maxValue=fallback or 0 for index=2,#numbers,3
do maxValue=math.max(maxValue,numbers[index]or maxValue)end return
NumberSequence.new(maxValue)end local function descriptorColorSequence(value,
fallback)if type(value)~='string'or value==''then return ColorSequence.new(
fallback or Color3.new(1,1,1))end local numbers=parseDescriptorNumbers(value)if#
numbers>=4 then return ColorSequence.new(Color3.new(math.clamp(numbers[2]or 1,0,
1),math.clamp(numbers[3]or 1,0,1),math.clamp(numbers[4]or 1,0,1)))end return
ColorSequence.new(fallback or Color3.new(1,1,1))end local function 
descriptorContent(value)if type(value)~='string'then return nil end if value==''
or value=='null'then return nil end return value end local function safeSet(
instance,property,value)if value==nil then return end pcall(function()instance[
property]=value end)end local function safeSetWithHiddenFallback(instance,
property,value)if value==nil then return false end local ok=pcall(function()
instance[property]=value end)if ok then return true end local setHidden=
getGlobalFunction('sethiddenproperty')if type(setHidden)=='function'then local
hiddenOk=pcall(setHidden,instance,property,value)if hiddenOk then return true
end end return false end local function safeGet(instance,property,fallback)local
ok,value=pcall(function()return instance[property]end)if ok and value~=nil then
return value end return fallback end local function safeSetAttribute(instance,
name,value)if instance==nil or type(name)~='string'or name==''then return end if
type(value)=='string'or type(value)=='number'or type(value)=='boolean'then
pcall(function()instance:SetAttribute(name,value)end)elseif type(value)=='table'
and#value==3 then pcall(function()instance:SetAttribute(name,Vector3.new(
tonumber(value[1])or 0,tonumber(value[2])or 0,tonumber(value[3])or 0))end)end
end local function applyDescriptorAttributes(instance,attributes)if type(
attributes)~='table'then return end for key,value in pairs(attributes)do
safeSetAttribute(instance,tostring(key),value)end end local function 
descriptorEnum(enumType,value,fallback)if value==nil or enumType==nil then
return fallback end if typeof(value)=='EnumItem'then return value end local
numeric=tonumber(value)local text=tostring(value)local shortName=text:match(
'%.([%w_]+)$')or text local ok,items=pcall(function()return enumType:
GetEnumItems()end)if not ok or type(items)~='table'then return fallback end for
_,item in ipairs(items)do if item.Name==shortName then return item end local
valueOk,itemValue=pcall(function()return item.Value end)if numeric~=nil and
valueOk and tonumber(itemValue)==numeric then return item end end return
fallback end local function applyBasePartDescriptor(part,properties,anchorPart,
localCFrame)safeSet(part,'Size',descriptorVector3(properties.size or properties.
Size,Vector3.new(1,1,1)))safeSet(part,'Color',descriptorColor(properties.
Color3uint8 or properties.Color,Color3.new(1,1,1)))safeSet(part,'Transparency',
math.clamp(tonumber(properties.Transparency)or 0,0,1))safeSet(part,'Material',
descriptorEnum(Enum.Material,properties.Material,Enum.Material.SmoothPlastic))
safeSet(part,'Reflectance',tonumber(properties.Reflectance)or 0)safeSet(part,
'CanCollide',false)safeSet(part,'CanTouch',false)safeSet(part,'CanQuery',false)
safeSet(part,'Massless',true)safeSet(part,'Anchored',false)safeSet(part,
'TopSurface',Enum.SurfaceType.Smooth)safeSet(part,'BottomSurface',Enum.
SurfaceType.Smooth)if anchorPart then part.CFrame=anchorPart.CFrame*(localCFrame
or descriptorCFrame(properties.LocalCFrame))end end local function 
createDescriptorMeshPart(node)local properties=type(node.properties)=='table'and
node.properties or{}local meshId=descriptorContent(properties.MeshId)local ok,
meshPart=pcall(Instance.new,'MeshPart')if ok and meshPart then return meshPart
end if AssetService and meshId and type(AssetService.CreateMeshPartAsync)==
'function'then local fidelity=descriptorEnum(Enum.RenderFidelity,properties.
RenderFidelity,Enum.RenderFidelity.Precise)local createdOk,created=pcall(
function()return AssetService:CreateMeshPartAsync(meshId,Enum.CollisionFidelity.
Default,fidelity)end)if createdOk and created then return created end end local
fallback=Instance.new('Part')safeSetAttribute(fallback,'OverlaySourceClass',
'MeshPart')safeSetAttribute(fallback,'OverlayMeshPartFallback',true)return
fallback end local function createDescriptorInstance(node)local className=node.
class if className=='Folder'then return Instance.new('Folder')elseif className==
'Model'then return Instance.new('Model')elseif className=='Part'then return
Instance.new('Part')elseif className=='WedgePart'or className=='CornerWedgePart'
or className=='TrussPart'or className=='UnionOperation'then local ok,instance=
pcall(Instance.new,className)return ok and instance or Instance.new('Part')
elseif className=='MeshPart'then return createDescriptorMeshPart(node)elseif
className=='SpecialMesh'then return Instance.new('SpecialMesh')elseif className
=='ParticleEmitter'then return Instance.new('ParticleEmitter')elseif className==
'Attachment'then return Instance.new('Attachment')elseif className=='Bone'then
local ok,instance=pcall(Instance.new,'Bone')return ok and instance or nil elseif
className=='Highlight'then return Instance.new('Highlight')elseif className==
'Decal'then return Instance.new('Decal')elseif className=='Texture'then return
Instance.new('Texture')elseif className=='PointLight'then return Instance.new(
'PointLight')elseif className=='SpotLight'then return Instance.new('SpotLight')
elseif className=='SurfaceLight'then return Instance.new('SurfaceLight')elseif
className=='SurfaceAppearance'or className=='WrapLayer'or className==
'WrapTarget'then local ok,instance=pcall(Instance.new,className)return ok and
instance or nil elseif className=='Weld'or className=='Motor6D'or className==
'WeldConstraint'then local ok,instance=pcall(Instance.new,className)return ok
and instance or nil end return nil end local function 
applyAttachmentLikeDescriptor(instance,properties)if properties.CFrame then
safeSet(instance,'CFrame',descriptorCFrame(properties.CFrame))else safeSet(
instance,'Position',descriptorVector3(properties.Position,Vector3.new()))
safeSet(instance,'Orientation',descriptorVector3(properties.Orientation,Vector3.
new()))end safeSet(instance,'Axis',descriptorVector3(properties.Axis,safeGet(
instance,'Axis',Vector3.new(1,0,0))))safeSet(instance,'SecondaryAxis',
descriptorVector3(properties.SecondaryAxis,safeGet(instance,'SecondaryAxis',
Vector3.new(0,1,0))))end local function applyDescriptorInstanceProperties(
instance,node,anchorPart,localCFrame)local properties=type(node.properties)==
'table'and node.properties or{}safeSet(instance,'Name',tostring(node.name or
properties.Name or node.class or'OverlayNode'))applyDescriptorAttributes(
instance,node.attributes)if instance:IsA('BasePart')then
applyBasePartDescriptor(instance,properties,anchorPart,localCFrame)if tostring(
node.name or properties.Name or'')=='Middle'then safeSet(instance,'Transparency'
,1)safeSet(instance,'CanQuery',false)end if node.class=='MeshPart'then local
meshId=descriptorContent(properties.MeshId)local textureId=descriptorContent(
properties.TextureID or properties.TextureId)if instance:IsA('MeshPart')then
safeSetWithHiddenFallback(instance,'MeshId',meshId)safeSetWithHiddenFallback(
instance,'TextureID',textureId)safeSet(instance,'RenderFidelity',descriptorEnum(
Enum.RenderFidelity,properties.RenderFidelity,Enum.RenderFidelity.Precise))
safeSet(instance,'Size',descriptorVector3(properties.size or properties.Size,
Vector3.new(1,1,1)))elseif meshId then local mesh=instance:FindFirstChild(
'OverlayMeshPartFallback')if mesh==nil or not mesh:IsA('SpecialMesh')then mesh=
Instance.new('SpecialMesh')mesh.Name='OverlayMeshPartFallback'mesh.Parent=
instance end safeSet(mesh,'MeshType',Enum.MeshType.FileMesh)safeSet(mesh,
'MeshId',meshId)safeSet(mesh,'TextureId',textureId)safeSet(mesh,'Scale',Vector3.
new(1,1,1))safeSet(mesh,'Offset',Vector3.new())else safeSet(instance,
'Transparency',1)end end elseif instance:IsA('SpecialMesh')then local meshId=
descriptorContent(properties.MeshId)safeSet(instance,'MeshId',meshId)safeSet(
instance,'TextureId',descriptorContent(properties.TextureId))safeSet(instance,
'Scale',descriptorVector3(properties.Scale,Vector3.new(1,1,1)))safeSet(instance,
'Offset',descriptorVector3(properties.Offset,Vector3.new()))safeSet(instance,
'VertexColor',descriptorVector3(properties.VertexColor,Vector3.new(1,1,1)))if
meshId then safeSet(instance,'MeshType',Enum.MeshType.FileMesh)else safeSet(
instance,'MeshType',descriptorEnum(Enum.MeshType,properties.MeshType,safeGet(
instance,'MeshType',Enum.MeshType.Head)))end elseif instance:IsA(
'ParticleEmitter')then safeSet(instance,'Texture',descriptorContent(properties.
Texture))safeSet(instance,'Enabled',properties.Enabled~=false)safeSet(instance,
'Rate',tonumber(properties.Rate)or 8)safeSet(instance,'Lifetime',
descriptorNumberRange(properties.Lifetime,0.5,1))safeSet(instance,'Speed',
descriptorNumberRange(properties.Speed,0,1))safeSet(instance,'SpreadAngle',
descriptorVector2(properties.SpreadAngle,Vector2.new(0,0)))safeSet(instance,
'Color',descriptorColorSequence(properties.Color,Color3.new(1,1,1)))safeSet(
instance,'Size',descriptorNumberSequence(properties.Size,0.2))safeSet(instance,
'Transparency',descriptorNumberSequence(properties.Transparency,0))safeSet(
instance,'LightEmission',tonumber(properties.LightEmission)or 0)safeSet(instance
,'LightInfluence',tonumber(properties.LightInfluence)or 0)safeSet(instance,
'Acceleration',descriptorVector3(properties.Acceleration,Vector3.new()))safeSet(
instance,'Drag',tonumber(properties.Drag)or 0)safeSet(instance,'LockedToPart',
properties.LockedToPart==true)safeSet(instance,'TimeScale',tonumber(properties.
TimeScale)or 1)safeSet(instance,'ZOffset',tonumber(properties.ZOffset)or 0)
...
safeSet(instance,'VelocityInheritance',tonumber(properties.VelocityInheritance)
or 0)elseif instance:IsA('Attachment')then applyAttachmentLikeDescriptor(
instance,properties)elseif instance:IsA('Bone')then
applyAttachmentLikeDescriptor(instance,properties)safeSet(instance,'Transform',
descriptorCFrame(properties.Transform))safeSet(instance,'Visible',properties.
Visible==true)elseif instance:IsA('Highlight')then safeSet(instance,'Enabled',
properties.Enabled~=false)safeSet(instance,'FillColor',descriptorColor(
properties.FillColor,Color3.new(1,1,1)))safeSet(instance,'OutlineColor',
descriptorColor(properties.OutlineColor,Color3.new(1,1,1)))safeSet(instance,
'FillTransparency',math.clamp(tonumber(properties.FillTransparency)or 0.8,0,1))
safeSet(instance,'OutlineTransparency',math.clamp(tonumber(properties.
OutlineTransparency)or 0,0,1))safeSet(instance,'DepthMode',descriptorEnum(Enum.
HighlightDepthMode,properties.DepthMode,safeGet(instance,'DepthMode',Enum.
HighlightDepthMode.Occluded)))elseif instance:IsA('Decal')or instance:IsA(
'Texture')then safeSet(instance,'Texture',descriptorContent(properties.Texture))
safeSet(instance,'Transparency',math.clamp(tonumber(properties.Transparency)or 0
,0,1))safeSet(instance,'Color3',descriptorColor(properties.Color3,Color3.new(1,1
,1)))elseif instance:IsA('Light')then safeSet(instance,'Enabled',properties.
Enabled~=false)safeSet(instance,'Color',descriptorColor(properties.Color,Color3.
new(1,1,1)))safeSet(instance,'Brightness',tonumber(properties.Brightness)or 1)
safeSet(instance,'Range',tonumber(properties.Range)or 8)safeSet(instance,
'Shadows',properties.Shadows==true)elseif instance:IsA('SurfaceAppearance')then
safeSet(instance,'ColorMap',descriptorContent(properties.ColorMap))safeSet(
instance,'MetalnessMap',descriptorContent(properties.MetalnessMap))safeSet(
instance,'NormalMap',descriptorContent(properties.NormalMap))safeSet(instance,
'RoughnessMap',descriptorContent(properties.RoughnessMap))elseif instance:IsA(
'WrapLayer')or instance:IsA('WrapTarget')then safeSet(instance,'CageMeshId',
descriptorContent(properties.CageMeshId))safeSet(instance,'ReferenceMeshId',
descriptorContent(properties.ReferenceMeshId))safeSet(instance,'Enabled',
properties.Enabled~=false)safeSet(instance,'Puffiness',tonumber(properties.
Puffiness)or 0)safeSet(instance,'Order',tonumber(properties.Order)or 0)end end
local function applyDescriptorLink(instance,node,instances)local properties=
type(node.properties)=='table'and node.properties or{}if instance:IsA('Weld')or
instance:IsA('Motor6D')or instance:IsA('WeldConstraint')then local part0=
properties.Part0 and instances[properties.Part0]or nil local part1=properties.
Part1 and instances[properties.Part1]or nil if part0 and part0:IsA('BasePart')
then safeSet(instance,'Part0',part0)end if part1 and part1:IsA('BasePart')then
safeSet(instance,'Part1',part1)end if instance:IsA('Weld')or instance:IsA(
'Motor6D')then safeSet(instance,'C0',descriptorCFrame(properties.C0))safeSet(
instance,'C1',descriptorCFrame(properties.C1))end if instance:IsA('Motor6D')then
safeSet(instance,'Transform',descriptorCFrame(properties.Transform))end safeSet(
instance,'Enabled',properties.Enabled~=false)end end local function 
descriptorAnchorCandidates(descriptor)local candidates={}if type(descriptor)==
'table'and type(descriptor.anchor_candidates)=='table'then for _,name in ipairs(
descriptor.anchor_candidates)do if type(name)=='string'and name~=''then table.
insert(candidates,name)end end end if type(descriptor)=='table'and type(
descriptor.anchor)=='string'and descriptor.anchor~=''then table.insert(
candidates,descriptor.anchor)end return candidates end local function 
findCharacterPart(character,candidates)if character==nil then return nil end
local aliases={UpperTorso={'UpperTorso','Torso','HumanoidRootPart'},LowerTorso={
'LowerTorso','Torso','HumanoidRootPart','UpperTorso'},Torso={'Torso',
'UpperTorso','LowerTorso','HumanoidRootPart'},Head={'Head','UpperTorso','Torso',
'HumanoidRootPart'},HumanoidRootPart={'HumanoidRootPart','LowerTorso',
'UpperTorso','Torso','Head'},['Left Arm']={'Left Arm','LeftUpperArm',
'LeftLowerArm','LeftHand','UpperTorso','Torso'},['Right Arm']={'Right Arm',
'RightUpperArm','RightLowerArm','RightHand','UpperTorso','Torso'},['Left Leg']={
'Left Leg','LeftUpperLeg','LeftLowerLeg','LeftFoot','LowerTorso','Torso'},[
'Right Leg']={'Right Leg','RightUpperLeg','RightLowerLeg','RightFoot',
'LowerTorso','Torso'},RightHand={'RightHand','Right Arm','RightLowerArm',
'RightUpperArm','UpperTorso','Torso'},LeftHand={'LeftHand','Left Arm',
'LeftLowerArm','LeftUpperArm','UpperTorso','Torso'},RightLowerArm={
'RightLowerArm','Right Arm','RightUpperArm','RightHand','UpperTorso','Torso'},
LeftLowerArm={'LeftLowerArm','Left Arm','LeftUpperArm','LeftHand','UpperTorso',
'Torso'},RightUpperArm={'RightUpperArm','Right Arm','RightLowerArm','RightHand',
'UpperTorso','Torso'},LeftUpperArm={'LeftUpperArm','Left Arm','LeftLowerArm',
'LeftHand','UpperTorso','Torso'},RightFoot={'RightFoot','Right Leg',
'RightLowerLeg','RightUpperLeg','LowerTorso','Torso'},LeftFoot={'LeftFoot',
'Left Leg','LeftLowerLeg','LeftUpperLeg','LowerTorso','Torso'},RightLowerLeg={
'RightLowerLeg','Right Leg','RightUpperLeg','RightFoot','LowerTorso','Torso'},
LeftLowerLeg={'LeftLowerLeg','Left Leg','LeftUpperLeg','LeftFoot','LowerTorso',
'Torso'},RightUpperLeg={'RightUpperLeg','Right Leg','RightLowerLeg','RightFoot',
'LowerTorso','Torso'},LeftUpperLeg={'LeftUpperLeg','Left Leg','LeftLowerLeg',
'LeftFoot','LowerTorso','Torso'}}local tried={}local function tryName(name)if
tried[name]then return nil end tried[name]=true local part=character:
FindFirstChild(name)if part and part:IsA('BasePart')then return part end return
nil end for _,candidate in ipairs(candidates or{})do local list=aliases[
candidate]or{candidate}for _,name in ipairs(list)do local part=tryName(name)if
part then return part end end end return tryName('HumanoidRootPart')or tryName(
'UpperTorso')or tryName('Torso')or tryName('Head')end local function 
resolveMorphAnchor(character,descriptor,fallback)return findCharacterPart(
character,descriptorAnchorCandidates(descriptor))or fallback end local
MORPH_GROUP_MOUNTS={HeadAcc={anchor='Head'},TorsoAcc={anchor='Torso'},
UpperStatic={anchor='Torso'},UpperStaticA={anchor='Torso'},UpperStaticB={anchor=
'Torso'},LowerStatic={anchor='Torso'},LowerStaticA={anchor='Torso'},LowerStaticB
={anchor='Torso'},RearStatic={anchor='Torso'},Cage={anchor='Torso'},ArmL={anchor
='Left Arm'},ArmR={anchor='Right Arm'},LegL={anchor='Left Leg'},LegR={anchor=
'Right Leg'},BodyLeftArm={anchor='Left Arm'},BodyRightArm={anchor='Right Arm'},
BodyLeftLeg={anchor='Left Leg'},BodyRightLeg={anchor='Right Leg'},BodyTorso={
anchor='Torso'},Rod={anchor='Torso',offset='torso_bottom_front'},Orbs={anchor=
'Torso',offset='torso_bottom_front'},UpperL={anchor='Torso',offset=
'torso_upper_left_front'},UpperR={anchor='Torso',offset=
'torso_upper_right_front'},LowerL={anchor='Torso',offset='torso_lower_left_back'
},LowerR={anchor='Torso',offset='torso_lower_right_back'},AccRod={anchorGroup=
'Rod',anchorPart='M6DRod'}}local NativeMorph local loadNativeMorphArchive local
applyDescriptorMorph local clearDescriptorMorph do NativeMorph={groupOrder={
'Rod','Orbs','UpperL','UpperR','LowerL','LowerR','UpperStatic','UpperStaticA',
'UpperStaticB','LowerStatic','LowerStaticA','LowerStaticB','RearStatic',
'TorsoAcc','AccRod','Cage','HeadAcc','ArmL','ArmR','LegL','LegR','BodyLeftArm',
'BodyRightArm','BodyLeftLeg','BodyRightLeg','BodyTorso','Face'},slotLabels={['1'
]='1 Pink',['2']='2 Green',['3']='3 Purple',['4']='4 Blue',['5']='5 Orange',['6'
]='6 Yellow',['7']='7 Red',['8']='8 Cyan',['9']='Customize 1',['10']=
'Customize 2',default='General'},slotAliases={['1']='1',['1pink']='1',pink='1',
upper='1',chest='1',['2']='2',['2green']='2',green='2',lower='2',rear='2',['3']=
'3',['3purple']='3',purple='3',['4']='4',['4blue']='4',blue='4',front='4',['5']=
'5',['5orange']='5',orange='5',['6']='6',['6yellow']='6',yellow='6',body='6',[
'7']='7',['7red']='7',red='7',accessory='7',['8']='8',['8cyan']='8',cyan='8',
head='8',['9']='9',['customize1']='9',customize1='9',['10']='10',['customize2']=
'10',customize2='10'}}function NativeMorph.normalizeSlot(value)local raw=
trimString(value)if raw==''then return nil end local firstSegment=string.match(
raw,'^([^/]+)/')if firstSegment then raw=firstSegment end local key=string.
lower(raw):gsub('[%s%p_%-]+','')return NativeMorph.slotAliases[key]or(
NativeMorph.slotLabels[raw]and raw or nil)end local function 
isDescriptorBasePartClass(className)return className=='Part'or className==
'WedgePart'or className=='CornerWedgePart'or className=='TrussPart'or className
=='UnionOperation'or className=='MeshPart'end local function 
descriptorRuntimePlan(descriptor)if type(descriptor)~='table'then return{
nodesById={},childrenByParent={},mountByNode={}}end if descriptor._runtimePlan
then return descriptor._runtimePlan end local plan={nodesById={},
childrenByParent={},mountByGroup={},mountByNode={}}for _,node in ipairs(
descriptor.nodes or{})do if type(node)=='table'and node.id then plan.nodesById[
node.id]=node local parent=node.parent or'__root'plan.childrenByParent[parent]=
plan.childrenByParent[parent]or{}table.insert(plan.childrenByParent[parent],node
)end end for _,node in ipairs(descriptor.nodes or{})do local mount=
MORPH_GROUP_MOUNTS[tostring(node.name or'')]if mount then local middleNode=nil
for _,child in ipairs(plan.childrenByParent[node.id]or{})do if tostring(child.
name or'')=='Middle'and isDescriptorBasePartClass(child.class)then middleNode=
child break end end if middleNode then plan.mountByGroup[node.id]={group=node,
middle=middleNode,anchor=mount.anchor,offset=mount.offset,middleLocal=
descriptorCFrame(type(middleNode.properties)=='table'and middleNode.properties.
LocalCFrame or nil)}end end end local function nearestMount(node)local current=
node while current do local mount=plan.mountByGroup[current.id]if mount then
return mount end current=current.parent and plan.nodesById[current.parent]or nil
end return nil end for _,node in ipairs(descriptor.nodes or{})do if type(node)==
'table'and node.id then plan.mountByNode[node.id]=nearestMount(node)end end
descriptor._runtimePlan=plan return plan end local function morphMountOffset(
anchorPart,offsetKind)if anchorPart==nil or offsetKind==nil then return CFrame.
new()end local size=safeGet(anchorPart,'Size',Vector3.new(2,2,1))if offsetKind==
'torso_bottom_front'then return CFrame.new(0,-size.Y/2,-size.Z/2)elseif
offsetKind=='torso_upper_left_front'then return CFrame.new(-size.X/8,size.Y/2,-
size.Z/2)elseif offsetKind=='torso_upper_right_front'then return CFrame.new(size
.X/8,size.Y/2,-size.Z/2)elseif offsetKind=='torso_lower_left_back'then return
CFrame.new(-size.X/8,-size.Y/2,size.Z/2)elseif offsetKind==
'torso_lower_right_back'then return CFrame.new(size.X/8,-size.Y/2,size.Z/2)end
return CFrame.new()end local function descriptorNodePlacement(plan,node,
character,fallbackAnchor)local properties=type(node.properties)=='table'and node
.properties or{}local nodeLocal=descriptorCFrame(properties.LocalCFrame)local
mount=plan.mountByNode[node.id]if mount then local anchor=findCharacterPart(
character,{mount.anchor})or fallbackAnchor local relativeToMiddle=mount.
middleLocal:Inverse()*nodeLocal return anchor,morphMountOffset(anchor,mount.
offset)*relativeToMiddle end return fallbackAnchor,nodeLocal end local
SMART_BONE_DEFAULTS={Damping=0.1,Stiffness=0.2,Inertia=0,Elasticity=0.5,
...
attributeNumber(rootPart,{'BlendWeight','blendWeight'},SMART_BONE_DEFAULTS.
BlendWeight,0,1),Radius=attributeNumber(rootPart,{'Radius','radius'},
SMART_BONE_DEFAULTS.Radius,0,10),AnchorDepth=math.floor(attributeNumber(rootPart
,{'AnchorDepth','anchorDepth'},SMART_BONE_DEFAULTS.AnchorDepth,0,12)),
AnchorsRotate=attributeValue(rootPart,{'AnchorsRotate','anchorsRotate'})==true,
Gravity=attributeVector(rootPart,{'Gravity','gravity'},SMART_BONE_DEFAULTS.
Gravity),Force=attributeVector(rootPart,{'Force','force'},SMART_BONE_DEFAULTS.
Force),WindDirection=attributeVector(rootPart,{'WindDirection','windDirection'},
SMART_BONE_DEFAULTS.WindDirection),WindSpeed=attributeNumber(rootPart,{
'WindSpeed','windSpeed'},SMART_BONE_DEFAULTS.WindSpeed,0,30),WindStrength=
attributeNumber(rootPart,{'WindStrength','windStrength'},SMART_BONE_DEFAULTS.
WindStrength,0,10),WindInfluence=attributeNumber(rootPart,{'WindInfluence',
'windInfluence'},SMART_BONE_DEFAULTS.WindInfluence,0,10),UpdateRate=
attributeNumber(rootPart,{'UpdateRate','updateRate'},SMART_BONE_DEFAULTS.
UpdateRate,1,165),ActivationDistance=attributeNumber(rootPart,{
'ActivationDistance','activationDistance'},SMART_BONE_DEFAULTS.
ActivationDistance,1,500),ThrottleDistance=attributeNumber(rootPart,{
'ThrottleDistance','throttleDistance'},SMART_BONE_DEFAULTS.ThrottleDistance,0,
500)}end local function findDescendantBoneByName(rootPart,name)name=trimString(
name)if name==''then return nil end for _,descendant in ipairs(rootPart:
GetDescendants())do if descendant:IsA('Bone')and descendant.Name==name then
return descendant end end return nil end local function childBones(bone)local
out={}for _,child in ipairs(bone:GetChildren())do if child:IsA('Bone')then table
.insert(out,child)end end return out end NativeMorph.smartBoneSignalAttributes={
'SmartBone','smartBone','smartbone','Jiggle','jiggle','PhysicsBone',
'physicsBone'}NativeMorph.smartBoneSettingAttributes={'Roots','Root','Damping',
'Stiffness','Inertia','Elasticity','BlendWeight','Radius','AnchorDepth',
'AnchorsRotate','Force','Gravity','WindDirection','WindSpeed','WindStrength',
'WindInfluence','UpdateRate','ActivationDistance','ThrottleDistance'}function
NativeMorph.hasCollectionTag(instance,tag)if CONFIG.collection_service==nil or
instance==nil then return false end local ok,result=pcall(function()return
CONFIG.collection_service:HasTag(instance,tag)end)return ok and result==true end
function NativeMorph.hasSmartBoneSignal(rootPart)if rootPart==nil then return
false end if NativeMorph.hasCollectionTag(rootPart,'SmartBone')then return true
end local rootsRaw=attributeValue(rootPart,{'Roots','roots','Root','root'})if
type(rootsRaw)=='string'and trimString(rootsRaw)~=''then return true end for _,
name in ipairs(NativeMorph.smartBoneSignalAttributes)do local value=
attributeValue(rootPart,{name})if value~=nil and value~=false and trimString(
tostring(value))~=''then return true end end for _,name in ipairs(NativeMorph.
smartBoneSettingAttributes)do if attributeValue(rootPart,{name})~=nil then
return true end end return false end function NativeMorph.inferSmartBoneRoots(
rootPart)local roots={}local firstBone=nil for _,descendant in ipairs(rootPart:
GetDescendants())do if descendant:IsA('Bone')then firstBone=firstBone or
descendant local hasBoneParent=false local parent=descendant.Parent while parent
and parent~=rootPart do if parent:IsA('Bone')then hasBoneParent=true break end
parent=parent.Parent end if not hasBoneParent then table.insert(roots,descendant
)if#roots>=8 then break end end end end if#roots==0 and firstBone then table.
insert(roots,firstBone)end return roots end function NativeMorph.
smartBoneRootBones(rootPart)local roots={}local rootsRaw=attributeValue(rootPart
,{'Roots','roots','Root','root'})if type(rootsRaw)=='string'and trimString(
rootsRaw)~=''then for rootName in string.gmatch(rootsRaw,'([^,]+)')do local
rootBone=findDescendantBoneByName(rootPart,rootName)if rootBone then table.
insert(roots,rootBone)end end if#roots>0 then return roots end end if CONFIG.
...
Position-controller.rootPart.Position).Magnitude end local activationDistance=
math.max(settings.ActivationDistance or SMART_BONE_DEFAULTS.ActivationDistance,1
)local throttleDistance=math.max(settings.ThrottleDistance or
SMART_BONE_DEFAULTS.ThrottleDistance,0)local updateDistance=math.clamp(distance-
...
'Native morph archive path is empty'return nil,Client.nativeMorphArchiveError
end local isfileFn=getGlobalFunction('isfile')if not isfileFn then Client.
nativeMorphArchiveStatus='filesystem_unavailable'Client.nativeMorphArchiveError=
'Executor filesystem API is missing'return nil,Client.nativeMorphArchiveError
end local existsOk,exists=pcall(isfileFn,path)if existsOk and exists then Client
.nativeMorphArchiveStatus='cached'Client.nativeMorphArchiveError=nil return path
,'cached'end local url=trimString(CONFIG.native_morph_archive_url)local
writefileFn=getGlobalFunction('writefile')if url==''then Client.
nativeMorphArchiveStatus='missing'Client.nativeMorphArchiveError=
[[Native morph archive is missing and no download URL is configured]]return nil,
Client.nativeMorphArchiveError end if not writefileFn then Client.
nativeMorphArchiveStatus='write_unavailable'Client.nativeMorphArchiveError=
'Executor writefile API is missing'return nil,Client.nativeMorphArchiveError end
local folder=parentFolder(path)if folder~=''and not ensureFolder(folder)then
Client.nativeMorphArchiveStatus='folder_failed'Client.nativeMorphArchiveError=
'Could not create native morph cache folder'return nil,Client.
nativeMorphArchiveError end Client.nativeMorphArchiveStatus='downloading'Client.
nativeMorphArchiveError=nil emitBridgeState('native morph archive downloading')
log('downloading native morph archive',url)local body=responseBody(requestHttp(
url))if type(body)~='string'or#body==0 then Client.nativeMorphArchiveStatus=
'download_failed'Client.nativeMorphArchiveError=
'Native morph archive download failed'return nil,Client.nativeMorphArchiveError
end if#body>CONFIG.native_morph_archive_max_bytes then Client.
nativeMorphArchiveStatus='too_large'Client.nativeMorphArchiveError=
'Native morph archive is too large'return nil,Client.nativeMorphArchiveError end
local writeOk=pcall(writefileFn,path,body)if not writeOk then Client.
nativeMorphArchiveStatus='write_failed'Client.nativeMorphArchiveError=
'Native morph archive could not be written to workspace'return nil,Client.
nativeMorphArchiveError end Client.nativeMorphArchiveStatus='downloaded'Client.
nativeMorphArchiveError=nil emitBridgeState('native morph archive cached: '..
tostring(#body)..' bytes')return path,'downloaded'end local function 
emitNativeMorphCatalog(archive)if archive==nil or Client.
nativeMorphCatalogEmitted then return end local assets={}local index={}local
slots={}Client.nativeMorphLookup={}local function addMorph(categoryName,item)if
item==nil or(not item:IsA('Folder')and not item:IsA('Model'))then return end
local pathName=trimString(categoryName)~=''and(categoryName..'/'..item.Name)or
item.Name local slot=NativeMorph.normalizeSlot(categoryName)or NativeMorph.
normalizeSlot(pathName)or'default'slots[slot]=NativeMorph.slotLabels[slot]or
slot table.insert(assets,{asset_id='native:'..pathName,name=item.Name,
display_name=pathName,category=categoryName,slot=slot,slot_label=NativeMorph.
slotLabels[slot]or slot,type='native_morph',kind='native_morph',format=
'rbxm_native_archive',source='native-rbxm'})local normalizedPath=
normalizeMorphName(pathName)local normalizedName=normalizeMorphName(item.Name)
table.insert(index,{instance=item,normalizedPath=normalizedPath,normalizedName=
normalizedName,displayName=pathName,slot=slot})Client.nativeMorphLookup[
normalizedPath]=item if Client.nativeMorphLookup[normalizedName]==nil then
Client.nativeMorphLookup[normalizedName]=item end end local startedAt=os.clock()
for _,category in ipairs(archive:GetChildren())do if category:IsA('Folder')or
category:IsA('Model')then local children=category:GetChildren()if#children>0
then for _,item in ipairs(children)do addMorph(category.Name,item)startedAt=
yieldIfBudgetSpent(startedAt)end else addMorph('',category)end end startedAt=
yieldIfBudgetSpent(startedAt)end table.sort(assets,function(a,b)return tostring(
a.display_name):lower()<tostring(b.display_name):lower()end)Client.
nativeMorphCatalog=assets Client.nativeMorphCatalogIndex=index Client.
nativeMorphCatalogSlots=slots Client.assetCatalog=assets Client.assetCatalogById
={}for _,asset in ipairs(assets)do Client.assetCatalogById[asset.asset_id]=asset
end Client.nativeMorphCatalogEmitted=true emitBridgeEvent('asset.catalog',{
assets=assets,total=#assets,source='native_morph_archive',status=Client.
nativeMorphArchiveStatus,path=CONFIG.native_morph_archive_path,slots=slots})
emitBridgeState('native morph catalog loaded: '..tostring(#assets))end
loadNativeMorphArchive=function()if typeof(Client.nativeMorphArchive)==
'Instance'then emitNativeMorphCatalog(Client.nativeMorphArchive)return Client.
nativeMorphArchive end if typeof(GLOBAL.OverlayNativeMorphArchive)=='Instance'
then Client.nativeMorphArchive=GLOBAL.OverlayNativeMorphArchive
emitNativeMorphCatalog(Client.nativeMorphArchive)return Client.
nativeMorphArchive end if Client.nativeMorphLoadAttempted then return Client.
nativeMorphArchive end Client.nativeMorphLoadAttempted=true if Client.
nativeMorphArchiveLoading then return Client.nativeMorphArchive end Client.
nativeMorphArchiveLoading=true local path,pathError=
ensureNativeMorphArchiveFile()local getcustomassetFn=getGlobalFunction(
'getcustomasset')if not path or not getcustomassetFn or not game.GetObjects then
Client.nativeMorphArchiveLoading=false if pathError then emitBridgeError(
'native_morph.archive_unavailable',pathError)log(
'native morph archive unavailable:',pathError)end return nil end local assetOk,
assetUrl=pcall(getcustomassetFn,path)if not assetOk or type(assetUrl)~='string'
or assetUrl==''then Client.nativeMorphArchiveStatus='custom_asset_failed'Client.
nativeMorphArchiveError='getcustomasset failed for native morph archive'Client.
nativeMorphArchiveLoading=false emitBridgeError(
'native_morph.custom_asset_failed',Client.nativeMorphArchiveError)return nil end
local objectsOk,objects=pcall(function()return game:GetObjects(assetUrl)end)if
not objectsOk or type(objects)~='table'or#objects==0 then Client.
nativeMorphArchiveStatus='load_failed'Client.nativeMorphArchiveError=
'game:GetObjects returned no native morph archive objects'Client.
nativeMorphArchiveLoading=false emitBridgeError('native_morph.load_failed',
Client.nativeMorphArchiveError)return nil end Client.nativeMorphArchive=objects[
1]GLOBAL.OverlayNativeMorphArchive=Client.nativeMorphArchive Client.
nativeMorphArchiveStatus='loaded'Client.nativeMorphArchiveError=nil Client.
nativeMorphArchiveLoading=false log('native morph archive loaded',Client.
nativeMorphArchive.Name)emitNativeMorphCatalog(Client.nativeMorphArchive)return
Client.nativeMorphArchive end local function nativeMorphCandidateScore(instance)
if instance:IsA('Folder')then return 3 end if instance:IsA('Model')then return 2
end return 0 end local function findNativeMorphByName(query)local normalized=
normalizeMorphName(query)if normalized==''then return nil end if Client.
nativeMorphLookup[normalized]~=nil then return Client.nativeMorphLookup[
normalized]end local archive=loadNativeMorphArchive()if archive==nil then return
nil end local exact=nil local partial=nil local startedAt=os.clock()for _,item
in ipairs(Client.nativeMorphCatalogIndex or{})do local instance=item.instance if
instance and instance.Parent then local score=nativeMorphCandidateScore(instance
)local name=item.normalizedName or normalizeMorphName(instance.Name)local
fullName=item.normalizedPath or name if name==normalized or fullName==normalized
then if exact==nil or score>exact.score then exact={instance=instance,score=
score}end elseif string.find(name,normalized,1,true)or string.find(normalized,
name,1,true)or string.find(fullName,normalized,1,true)or string.find(normalized,
fullName,1,true)then if partial==nil or score>partial.score then partial={
instance=instance,score=score}end end end startedAt=yieldIfBudgetSpent(startedAt
)end local found=exact and exact.instance or(partial and partial.instance or nil
)Client.nativeMorphLookup[normalized]=found or false return found end local 
function collectBaseParts(root)local parts={}if root:IsA('BasePart')then table.
insert(parts,root)end local startedAt=os.clock()for index,descendant in ipairs(
root:GetDescendants())do if descendant:IsA('BasePart')then table.insert(parts,
descendant)end if index%250==0 then startedAt=yieldIfBudgetSpent(startedAt)end
end return parts end local function countBasePartsLimited(root,limit)limit=
tonumber(limit)or 0 if limit<=0 then local count=root:IsA('BasePart')and 1 or 0
local startedAt=os.clock()for index,descendant in ipairs(root:GetDescendants())
do if descendant:IsA('BasePart')then count=count+1 end if index%250==0 then
startedAt=yieldIfBudgetSpent(startedAt)end end return count,true end local count
=root:IsA('BasePart')and 1 or 0 if count>limit then return count,false end local
startedAt=os.clock()for index,descendant in ipairs(root:GetDescendants())do if
descendant:IsA('BasePart')then count=count+1 if count>limit then return count,
false end end if index%250==0 then startedAt=yieldIfBudgetSpent(startedAt)end
end return count,true end local function nativeMorphStats(source)local stats=
Client.nativeMorphStats[source]if stats then return stats end local count,
withinLimit=countBasePartsLimited(source,CONFIG.native_morph_max_base_parts)
stats={baseParts=count,withinLimit=withinLimit}Client.nativeMorphStats[source]=
stats return stats end local function destroyInstanceDeferred(instance)if
instance==nil then return end pcall(function()instance.Parent=nil end)task.
defer(function()pcall(function()instance:Destroy()end)end)end local function 
findDirectMiddle(root)local middle=root:FindFirstChild('Middle')if middle and
middle:IsA('BasePart')then return middle end for _,child in ipairs(root:
GetChildren())do if child.Name=='Middle'and child:IsA('BasePart')then return
child end end return nil end local function findMorphMiddle(root)return
findDirectMiddle(root)or root:FindFirstChild('Middle',true)end local function 
hasJointBetween(a,b)if a==nil or b==nil then return false end local function 
scan(root)for _,item in ipairs(root:GetChildren())do if item:IsA('JointInstance'
)then local part0=safeGet(item,'Part0',nil)local part1=safeGet(item,'Part1',nil)
if(part0==a and part1==b)or(part0==b and part1==a)then return true end elseif
item:IsA('WeldConstraint')then local part0=safeGet(item,'Part0',nil)local part1=
safeGet(item,'Part1',nil)if(part0==a and part1==b)or(part0==b and part1==a)then
return true end end end return false end return scan(a)or scan(b)end local 
function ensureMiddleWelds(container)local middle=findDirectMiddle(container)if
middle then for _,child in ipairs(container:GetChildren())do if child:IsA(
'BasePart')and child~=middle and not hasJointBetween(middle,child)then local
weld=Instance.new('Weld')weld.Name='OverlayNativeInternalWeld'weld.Part0=middle
weld.Part1=child local center=CFrame.new(middle.Position)weld.C0=middle.CFrame:
Inverse()*center weld.C1=child.CFrame:Inverse()*center weld.Parent=middle end
end end for _,child in ipairs(container:GetChildren())do if child:IsA('Folder')
or child:IsA('Model')then ensureMiddleWelds(child)end end end local function 
topNativeMorphGroups(clone)local groups={}if MORPH_GROUP_MOUNTS[clone.Name]then
table.insert(groups,clone)end for _,groupName in ipairs(NativeMorph.groupOrder)
do local child=clone:FindFirstChild(groupName)if child and MORPH_GROUP_MOUNTS[
child.Name]then table.insert(groups,child)end end for _,child in ipairs(clone:
GetChildren())do if MORPH_GROUP_MOUNTS[child.Name]then local seen=false for _,
group in ipairs(groups)do if group==child then seen=true break end end if not
seen then table.insert(groups,child)end end end if#groups==0 then table.insert(
groups,clone)end return groups end local function moveMorphGroupByMiddle(group,
middle,desiredMiddleCFrame)local current=middle.CFrame local delta=
desiredMiddleCFrame*current:Inverse()for _,part in ipairs(collectBaseParts(group
))do part.CFrame=delta*part.CFrame end end function NativeMorph.colorFromAny(
value,fallback)if typeof(value)=='Color3'then return value end if type(value)==
'table'then return descriptorColor(value,fallback)end return fallback or Color3.
new(1,1,1)end function NativeMorph.characterBodyColor(character,fallback)local
part=findCharacterPart(character,{'Torso','UpperTorso','LowerTorso','Head'})if
part and part:IsA('BasePart')then return part.Color end return fallback or
Color3.new(1,1,1)end function NativeMorph.palette(character,components,entityId)
local fallback=colorFromComponents(tostring(entityId or'native_morph'),
components or{})local body=NativeMorph.characterBodyColor(character,fallback)
local morph=type(components)=='table'and components.morph or nil local colors=
type(morph)=='table'and morph.colors or nil return{body=body,color1=NativeMorph.
colorFromAny(colors and(colors.color1 or colors[1]),body),color2=NativeMorph.
colorFromAny(colors and(colors.color2 or colors[2]),body),color3=NativeMorph.
colorFromAny(colors and(colors.color3 or colors[3]),body),color4=NativeMorph.
colorFromAny(colors and(colors.color4 or colors[4]),body)}end function
NativeMorph.paletteKey(palette)local body=palette and palette.body or Color3.
new(1,1,1)local color1=palette and palette.color1 or body local color2=palette
and palette.color2 or body return table.concat({math.floor(body.R*255+0.5),math.
floor(body.G*255+0.5),math.floor(body.B*255+0.5),math.floor(color1.R*255+0.5),
math.floor(color1.G*255+0.5),math.floor(color1.B*255+0.5),math.floor(color2.R*
255+0.5),math.floor(color2.G*255+0.5),math.floor(color2.B*255+0.5)},',')end
function NativeMorph.hasMarker(root,markerName)if root==nil then return false
end local found=root:FindFirstChild(markerName,true)return found~=nil end
function NativeMorph.applyParticleColor(part,color)for _,child in ipairs(part:
GetChildren())do if child.Name=='Particles'or child:IsA('ParticleEmitter')or
child:IsA('Trail')or child:IsA('Beam')then safeSet(child,'Color',ColorSequence.
new(color))end end end function NativeMorph.forceColors(root,color)for _,
descendant in ipairs(root:GetDescendants())do if descendant:IsA('BasePart')then
safeSet(descendant,'Color',color)end end end function NativeMorph.setMarker(
instance,name,value)if instance==nil then return end pcall(function()instance:
SetAttribute(name,value)end)end function NativeMorph.isDirectCharacterMount(
character)return character~=nil and character:IsA('Model')and character:
FindFirstChildOfClass('Humanoid')~=nil end function NativeMorph.groupParent(
character,fallbackRoot)if NativeMorph.isDirectCharacterMount(character)then
return character end return fallbackRoot end function NativeMorph.
trackMountedGroup(state,group)if state==nil or group==nil then return end state.
mountedGroups=state.mountedGroups or{}table.insert(state.mountedGroups,group)end
function NativeMorph.trackMountedJoint(state,joint)if state==nil or joint==nil
then return end state.mountedJoints=state.mountedJoints or{}table.insert(state.
mountedJoints,joint)end function NativeMorph.findCharacterMorphGroup(character,
groupName)if not NativeMorph.isDirectCharacterMount(character)then return nil
end local child=character:FindFirstChild(groupName)if child and(child:IsA(
'Folder')or child:IsA('Model'))then return child end return nil end function
NativeMorph.findRodAnchor(character)local rod=NativeMorph.
findCharacterMorphGroup(character,'Rod')if rod==nil then return nil end local
anchor=rod:FindFirstChild('M6DRod')or rod:FindFirstChild('Middle')if anchor and
anchor:IsA('BasePart')then return anchor end return nil end function NativeMorph
.destroyTrackedMounts(state)if state==nil then return end for _,joint in ipairs(
state.mountedJoints or{})do if joint and joint.Parent then
destroyInstanceDeferred(joint)end end for _,group in ipairs(state.mountedGroups
or{})do if group and group.Parent then destroyInstanceDeferred(group)end end
state.mountedJoints=nil state.mountedGroups=nil end function NativeMorph.
groupConflictNames(groups)local groupNames={}for _,group in ipairs(groups or{})
do if group and MORPH_GROUP_MOUNTS[group.Name]then groupNames[group.Name]=true
end end if groupNames.Cage then groupNames.Rod=true groupNames.AccRod=true end
if groupNames.Rod or groupNames.AccRod then groupNames.Cage=true end return
groupNames end function NativeMorph.removeChildIfPresent(root,name)if root==nil
then return end local child=root:FindFirstChild(name)if child then
destroyInstanceDeferred(child)end end function NativeMorph.applyCageRules(handle
,clone,slot)if clone==nil or clone:FindFirstChild('Cage')==nil then return end
NativeMorph.removeChildIfPresent(clone,'AccRod')NativeMorph.
removeChildIfPresent(clone,'Rod')local slotsToClear={}for otherSlot,state in
pairs(handle.nativeMorphSlots or{})do if otherSlot~=slot and type(state.
groupNames)=='table'and(state.groupNames.Rod or state.groupNames.AccRod)then
table.insert(slotsToClear,otherSlot)end end for _,otherSlot in ipairs(
slotsToClear)do NativeMorph.clearSlot(handle,otherSlot)end end local function 
prepareNativeMorphPart(part,palette)safeSet(part,'Anchored',false)safeSet(part,
'CanCollide',false)safeSet(part,'CanTouch',false)safeSet(part,'CanQuery',false)
safeSet(part,'Massless',true)safeSet(part,'Transparency',0)palette=palette or{
body=part.Color,color1=part.Color,color2=part.Color,color3=part.Color,color4=
part.Color}if part.Name=='color1'or part:FindFirstChild('IsColor1')then safeSet(
part,'Color',palette.color1 or palette.body)elseif part.Name=='color2'or part:
FindFirstChild('IsColor2')then safeSet(part,'Color',palette.color2 or palette.
body)elseif part.Name=='color5'or part:FindFirstChild('IsColor3')or part:
FindFirstChild('IsColor5')then safeSet(part,'Color',palette.body)elseif part.
Name=='Particle1'then NativeMorph.applyParticleColor(part,palette.color3 or
palette.body)safeSet(part,'Transparency',1)elseif part.Name=='Particle2'or part.
Name=='Particle3'or part:FindFirstChild('IsColor4')then NativeMorph.
applyParticleColor(part,palette.color4 or palette.body)safeSet(part,
'Transparency',1)end if part.Name=='Middle'or part:FindFirstChild('Hidden')then
safeSet(part,'Transparency',1)safeSet(part,'LocalTransparencyModifier',1)end end
local function hideNativeMorphTarget(handle,part)if handle==nil or part==nil or
not part:IsA('BasePart')then return end handle.nativeMorphHiddenParts=handle.
nativeMorphHiddenParts or{}for _,item in ipairs(handle.nativeMorphHiddenParts)do
if item.part==part then return end end table.insert(handle.
nativeMorphHiddenParts,{part=part,transparency=safeGet(part,'Transparency',0),
localTransparencyModifier=safeGet(part,'LocalTransparencyModifier',0)})safeSet(
part,'Transparency',1)safeSet(part,'LocalTransparencyModifier',1)end local 
function restoreNativeMorphHiddenParts(handle)if handle==nil or type(handle.
nativeMorphHiddenParts)~='table'then return end for _,item in ipairs(handle.
nativeMorphHiddenParts)do local part=item and item.part if part and part.Parent
then safeSet(part,'Transparency',item.transparency)safeSet(part,
'LocalTransparencyModifier',item.localTransparencyModifier)end end handle.
nativeMorphHiddenParts=nil end function NativeMorph.slotState(handle,slot)if
handle==nil then return nil end slot=trimString(slot)~=''and trimString(slot)or
'default'handle.nativeMorphSlots=handle.nativeMorphSlots or{}local state=handle.
nativeMorphSlots[slot]if state==nil then state={slot=slot}handle.
nativeMorphSlots[slot]=state end return state end function NativeMorph.
rebuildSmartBones(handle)if handle==nil then return end local all={}for _,state
in pairs(handle.nativeMorphSlots or{})do for _,controller in ipairs(state.
smartBones or{})do table.insert(all,controller)end end handle.
nativeMorphSmartBones=#all>0 and all or nil end function NativeMorph.clearSlot(
handle,slot)if handle==nil then return end if type(handle.nativeMorphSlots)~=
'table'then return end local state=handle.nativeMorphSlots[slot]if state==nil
then return end restoreNativeMorphHiddenParts(state)NativeMorph.
destroyTrackedMounts(state)if state.root then destroyInstanceDeferred(state.root
)end handle.nativeMorphSlots[slot]=nil NativeMorph.rebuildSmartBones(handle)end
local function clearNativeMorph(handle)if handle==nil then return end if type(
handle.nativeMorphSlots)=='table'then local slots={}for slot in pairs(handle.
nativeMorphSlots)do table.insert(slots,slot)end for _,slot in ipairs(slots)do
NativeMorph.clearSlot(handle,slot)end end restoreNativeMorphHiddenParts(handle)
if handle.nativeMorphRoot then destroyInstanceDeferred(handle.nativeMorphRoot)
end handle.nativeMorphRoot=nil handle.nativeMorphKey=nil handle.
nativeMorphCharacter=nil handle.nativeMorphSmartBones=nil end local function 
mountNativeMorphGroup(handle,group,character,fallbackAnchor,slotState,palette,
root)local mount=MORPH_GROUP_MOUNTS[group.Name]local sourceRoot=group.Parent
local anchor=nil if mount and mount.anchorGroup=='Rod'then anchor=NativeMorph.
findRodAnchor(character)if anchor==nil then return false end elseif mount then
anchor=findCharacterPart(character,{mount.anchor})end anchor=anchor or
fallbackAnchor if anchor==nil or not anchor:IsA('BasePart')then return false end
ensureMiddleWelds(group)local middle=findMorphMiddle(group)if middle==nil or not
middle:IsA('BasePart')then return false end for _,part in ipairs(
collectBaseParts(group))do prepareNativeMorphPart(part,palette)end local offset=
mount and morphMountOffset(anchor,mount.offset)or CFrame.new()local
desiredMiddle=anchor.CFrame*offset moveMorphGroupByMiddle(group,middle,
desiredMiddle)local joint if mount and mount.offset then joint=Instance.new(
'Motor6D')joint.Name='M6D'..group.Name else joint=Instance.new('Weld')joint.Name
='OverlayNativeMorphWeld_'..group.Name end joint.Part0=anchor joint.Part1=middle
joint.C0=offset joint.C1=CFrame.new()joint.Parent=anchor NativeMorph.
trackMountedJoint(slotState,joint)if joint:IsA('Motor6D')then middle.Name='M6D'
..group.Name end NativeMorph.setMarker(group,'OverlayNativeMorphGroup',true)
group.Parent=NativeMorph.groupParent(character,root)NativeMorph.
trackMountedGroup(slotState,group)if mount and BODY_REPLACEMENT_TARGETS[group.
Name]then NativeMorph.forceColors(group,anchor.Color)if not NativeMorph.
hasMarker(sourceRoot,'IsNone')then hideNativeMorphTarget(slotState or handle,
anchor)end end return true end function NativeMorph.clearConflictingGroups(
handle,exceptSlot,groups)local groupNames=NativeMorph.groupConflictNames(groups)
local slotsToClear={}for slot,state in pairs(handle.nativeMorphSlots or{})do if
slot~=exceptSlot and type(state.groupNames)=='table'then for groupName in pairs(
groupNames)do if state.groupNames[groupName]then table.insert(slotsToClear,slot)
break end end end end for _,slot in ipairs(slotsToClear)do NativeMorph.
clearSlot(handle,slot)end return groupNames end local function 
createNativeSmartBoneControllers(root,extraRoots)if not CONFIG.
native_morph_enable_smart_bones then return nil end local maxControllers=
tonumber(CONFIG.native_morph_max_smart_bone_controllers)or 0 if maxControllers<=
0 then return nil end local controllers={}local roots={}if root then table.
insert(roots,root)end for _,extraRoot in ipairs(extraRoots or{})do if extraRoot
and extraRoot.Parent then table.insert(roots,extraRoot)end end for _,scanRoot in
ipairs(roots)do for _,part in ipairs(collectBaseParts(scanRoot))do local ok,
controller=pcall(createSmartBoneController,part)if not ok then controller=nil
end if controller then table.insert(controllers,controller)if#controllers>=
maxControllers then break end end end if#controllers>=maxControllers then break
end end return#controllers>0 and controllers or nil end function NativeMorph.
applySlot(handle,anchorPart,components,entityId,character,slot,morphEntry)if
handle==nil or type(morphEntry)~='table'then return false end local rawSlot=
trimString(slot or morphEntry.slot or morphEntry.category)slot=NativeMorph.
normalizeSlot(rawSlot)or(rawSlot~=''and rawSlot or'default')if morphEntry.
enabled==false then NativeMorph.clearSlot(handle,slot)return true end local
query=nativeMorphQuery(morphEntry)if query==nil then NativeMorph.clearSlot(
handle,slot)return false end local source=findNativeMorphByName(query)if source
==nil then NativeMorph.clearSlot(handle,slot)return true end local stats=
nativeMorphStats(source)if stats and not stats.withinLimit then NativeMorph.
clearSlot(handle,slot)local message='Native morph too heavy: '..tostring(stats.
baseParts)..' parts (limit '..tostring(CONFIG.native_morph_max_base_parts)..')'
emitBridgeError('native_morph.too_heavy',message)emitBridgeState(message)return
true end character=character or(anchorPart and anchorPart.Parent)local palette=
NativeMorph.palette(character,components,entityId)local key=tostring(slot)..':'
..normalizeMorphName(query)..':'..tostring(source)..':'..NativeMorph.paletteKey(
palette)local state=NativeMorph.slotState(handle,slot)if state.root and state.
key==key and state.character==character then return true end NativeMorph.
clearSlot(handle,slot)state=NativeMorph.slotState(handle,slot)local root=
Instance.new('Folder')root.Name='OverlayNativeMorph_'..tostring(entityId or
query)..'_'..tostring(slot)local clone=source:Clone()clone.Name=source.Name
clone.Parent=root NativeMorph.applyCageRules(handle,clone,slot)local groups=
topNativeMorphGroups(clone)local groupNames=NativeMorph.clearConflictingGroups(
handle,slot,groups)local mountedAny=false for _,group in ipairs(groups)do if
mountNativeMorphGroup(handle,group,character,anchorPart,state,palette,root)then
mountedAny=true end end if not mountedAny and anchorPart and anchorPart:IsA(
'BasePart')then local parts=collectBaseParts(clone)if#parts>0 then for _,part in
ipairs(parts)do prepareNativeMorphPart(part,palette)end local pivot=parts[1].
CFrame local target=anchorPart.CFrame for _,part in ipairs(parts)do part.CFrame=
target*pivot:Inverse()*part.CFrame local weld=Instance.new('WeldConstraint')weld
.Name='OverlayNativeFallbackWeld'weld.Part0=anchorPart weld.Part1=part weld.
Parent=part end mountedAny=true end end if not mountedAny then root:Destroy()
handle.nativeMorphSlots[slot]=nil return true end root.Parent=handle.folder or
entityFolder state.root=root state.key=key state.character=character state.
groupNames=groupNames state.smartBones=createNativeSmartBoneControllers(root,
state.mountedGroups)NativeMorph.rebuildSmartBones(handle)return true end local 
function applyNativeMorph(handle,anchorPart,components,entityId,character)if
handle==nil or type(components)~='table'then return false end local morph=
components.morph if type(morph)~='table'then return false end if morph.enabled==
false then clearNativeMorph(handle)return true end if type(morph.slots)=='table'
then if handle.descriptorMorphRoot then pcall(function()handle.
descriptorMorphRoot:Destroy()end)end handle.descriptorMorphRoot=nil handle.
descriptorMorphAssetId=nil handle.descriptorMorphAnchor=nil handle.
descriptorMorphJiggle=nil local activeSlots={}local appliedAny=false for slot,
entry in pairs(morph.slots)do local normalizedSlot=NativeMorph.normalizeSlot(
slot)or tostring(slot)activeSlots[normalizedSlot]=true if type(entry)=='table'
and entry.enabled~=false and nativeMorphQuery(entry)~=nil then if NativeMorph.
applySlot(handle,anchorPart,components,entityId,character,normalizedSlot,entry)
then appliedAny=true end else NativeMorph.clearSlot(handle,normalizedSlot)end
end local staleSlots={}for slot in pairs(handle.nativeMorphSlots or{})do if not
activeSlots[slot]then table.insert(staleSlots,slot)end end for _,slot in ipairs(
staleSlots)do NativeMorph.clearSlot(handle,slot)end return appliedAny or next(
activeSlots)~=nil end local query=nativeMorphQuery(morph)if query==nil then
return false end if handle.descriptorMorphRoot then pcall(function()handle.
descriptorMorphRoot:Destroy()end)end handle.descriptorMorphRoot=nil handle.
descriptorMorphAssetId=nil handle.descriptorMorphAnchor=nil handle.
descriptorMorphJiggle=nil local slot=NativeMorph.normalizeSlot(morph.slot or
morph.category or query)or'default'return NativeMorph.applySlot(handle,
anchorPart,components,entityId,character,slot,morph)end clearDescriptorMorph=
function(handle)clearNativeMorph(handle)if handle and handle.descriptorMorphRoot
then pcall(function()handle.descriptorMorphRoot:Destroy()end)end if handle then
handle.descriptorMorphRoot=nil handle.descriptorMorphAssetId=nil handle.
descriptorMorphAnchor=nil handle.descriptorMorphJiggle=nil handle.
descriptorMorphSmartBones=nil end end applyDescriptorMorph=function(handle,
anchorPart,components,entityId,character)if handle==nil or anchorPart==nil or
type(components)~='table'then return end local morph=components.morph local
assetId=assetIdFromComponent(morph)if assetId==nil or(type(morph)=='table'and
morph.enabled==false)then if type(morph)=='table'and morph.enabled~=false and
applyNativeMorph(handle,anchorPart,components,entityId,character)then return end
clearDescriptorMorph(handle)return end if applyNativeMorph(handle,anchorPart,
components,entityId,character)then return end local descriptor=
descriptorForAsset(assetId)if descriptor==nil then return end if handle.
descriptorMorphAssetId==assetId and handle.descriptorMorphAnchor==anchorPart and
handle.descriptorMorphRoot then return end clearDescriptorMorph(handle)local
root=Instance.new('Folder')root.Name='OverlayMorph_'..tostring(entityId or
assetId)root.Parent=handle.folder or entityFolder local instances={}local
createdNodes={}local jiggleBones={}local plan=descriptorRuntimePlan(descriptor)
for _,node in ipairs(descriptor.nodes or{})do local instance=
createDescriptorInstance(node)if instance then instances[node.id]=instance table
.insert(createdNodes,{node=node,instance=instance})local parent=instances[node.
parent]or root instance.Parent=parent local nodeAnchor,nodeLocalCFrame=
descriptorNodePlacement(plan,node,character,anchorPart)
applyDescriptorInstanceProperties(instance,node,nodeAnchor,nodeLocalCFrame)if
instance:IsA('BasePart')then local weld=Instance.new('WeldConstraint')weld.Name=
'OverlayDescriptorWeld'weld.Part0=nodeAnchor or anchorPart weld.Part1=instance
weld.Parent=instance elseif instance:IsA('Highlight')then safeSet(instance,
'Adornee',(nodeAnchor and nodeAnchor.Parent)or anchorPart.Parent or anchorPart)
elseif instance:IsA('Bone')then table.insert(jiggleBones,{bone=instance,base=
safeGet(instance,'Transform',CFrame.new()),phase=(#jiggleBones+1)*0.73,speed=4+(
#jiggleBones%4),amplitude=0.035})end end end for _,created in ipairs(
createdNodes)do applyDescriptorLink(created.instance,created.node,instances)end
handle.descriptorMorphRoot=root handle.descriptorMorphAssetId=assetId handle.
descriptorMorphAnchor=anchorPart if descriptor.jiggle==true and#jiggleBones>0
then handle.descriptorMorphJiggle=jiggleBones else handle.descriptorMorphJiggle=
nil end end end local function normalizeAnimationName(value)value=string.lower(
...
isfileFn then Client.nativeAnimationArchiveStatus='filesystem_unavailable'Client
.nativeAnimationArchiveError='Executor filesystem API is missing'return nil,
Client.nativeAnimationArchiveError end local existsOk,exists=pcall(isfileFn,path
)if existsOk and exists then Client.nativeAnimationArchiveStatus='cached'Client.
nativeAnimationArchiveError=nil return path,'cached'end local url=trimString(
CONFIG.native_animation_archive_url)local writefileFn=getGlobalFunction(
'writefile')if url==''then Client.nativeAnimationArchiveStatus='missing'Client.
nativeAnimationArchiveError=
[[Native animation archive is missing and no download URL is configured]]return
nil,Client.nativeAnimationArchiveError end if not writefileFn then Client.
nativeAnimationArchiveStatus='write_unavailable'Client.
nativeAnimationArchiveError='Executor writefile API is missing'return nil,Client
.nativeAnimationArchiveError end local folder=parentFolder(path)if folder~=''and
not ensureFolder(folder)then Client.nativeAnimationArchiveStatus='folder_failed'
Client.nativeAnimationArchiveError=
'Could not create native animation cache folder'return nil,Client.
nativeAnimationArchiveError end Client.nativeAnimationArchiveStatus=
'downloading'Client.nativeAnimationArchiveError=nil emitBridgeState(
'native animation archive downloading')log(
'downloading native animation archive',url)local body=responseBody(requestHttp(
url))if type(body)~='string'or#body==0 then Client.nativeAnimationArchiveStatus=
'download_failed'Client.nativeAnimationArchiveError=
'Native animation archive download failed'return nil,Client.
nativeAnimationArchiveError end if#body>CONFIG.
native_animation_archive_max_bytes then Client.nativeAnimationArchiveStatus=
'too_large'Client.nativeAnimationArchiveError=
'Native animation archive is too large'return nil,Client.
nativeAnimationArchiveError end local writeOk=pcall(writefileFn,path,body)if not
writeOk then Client.nativeAnimationArchiveStatus='write_failed'Client.
nativeAnimationArchiveError=
'Native animation archive could not be written to workspace'return nil,Client.
nativeAnimationArchiveError end Client.nativeAnimationArchiveStatus='downloaded'
Client.nativeAnimationArchiveError=nil emitBridgeState(
'native animation archive cached: '..tostring(#body)..' bytes')return path,
...
'native animation archive unavailable:',pathError)end return nil end local
assetOk,assetUrl=pcall(getcustomassetFn,path)if not assetOk or type(assetUrl)~=
'string'or assetUrl==''then Client.nativeAnimationArchiveStatus=
'custom_asset_failed'Client.nativeAnimationArchiveError=
'getcustomasset failed for native animation archive'Client.
nativeAnimationArchiveLoading=false emitBridgeError(
'native_animation.custom_asset_failed',Client.nativeAnimationArchiveError)return
nil end local objectsOk,objects=pcall(function()return game:GetObjects(assetUrl)
end)if not objectsOk or type(objects)~='table'or#objects==0 then Client.
nativeAnimationArchiveStatus='load_failed'Client.nativeAnimationArchiveError=
[[game:GetObjects returned no native animation archive objects]]Client.
nativeAnimationArchiveLoading=false emitBridgeError(
'native_animation.load_failed',Client.nativeAnimationArchiveError)return nil end
Client.nativeAnimationArchive=objects[1]GLOBAL.OverlayNativeAnimationArchive=
Client.nativeAnimationArchive Client.nativeAnimationArchiveStatus='loaded'Client
.nativeAnimationArchiveError=nil Client.nativeAnimationArchiveLoading=false log(
...
string.find(fullName,normalized,1,true)or string.find(normalized,fullName,1,true
)then partial=partial or instance end end end local found=exact or partial
Client.nativeAnimationLookup[normalized]=found or false return found end local 
function nativeAnimationEase(style,direction,percent)style=tostring(style or
'Linear')direction=tostring(direction or'Out')local function bounce(t)if t<
0.36363636 then return 7.5625*t*t elseif t<0.72727272 then t=t-0.54545454 return
7.5625*t*t+0.75 elseif t<0.9090909 then t=t-0.81818181 return 7.5625*t*t+0.9375
end t=t-0.95454545 return 7.5625*t*t+0.984375 end local function cubic(t)return
t^3 end local function easeIn(t,fn)return fn(t)end local function easeOut(t,fn)
return 1-fn(1-t)end local function easeInOut(t,fn)t=t*2 if t<1 then return
easeIn(t,fn)*0.5 end return 0.5+easeOut(t-1,fn)*0.5 end if style=='Bounce'then
if direction=='Out'then return 1-easeOut(percent,bounce)elseif direction=='In'
then return 1-bounce(percent)end return 1-easeInOut(percent,bounce)elseif style
=='Elastic'then local totalTime=1 if direction=='Out'then local p=totalTime*0.3
local t=1-percent local s=p/4 return 1+2^(-10*t)*math.sin((t*totalTime-s)*(math.
pi*2)/p)elseif direction=='In'then local p=totalTime*0.3 local t=percent local s
=p/4 return 1-(1+2^(-10*t)*math.sin((t*totalTime-s)*(math.pi*2)/p))end local t=
percent*2 local p=0.3*1.5 local s=p/4 if t<1 then t=t-1 return 1-(-0.5*2^(10*t)*
math.sin((t-s)*(math.pi*2)/p))end t=t-1 return 1-(1+0.5*2^(-10*t)*math.sin((t-s)
*(math.pi*2)/p))elseif style=='Cubic'then if direction=='Out'then return 1-
easeOut(percent,cubic)elseif direction=='In'then return 1-cubic(percent)end
return 1-easeInOut(percent,cubic)elseif style=='Constant'then if direction==
'Out'then return 1 elseif direction=='In'then return 0 end return 0.5 end return
1-percent end local function repairedAnimationCFrame(cf)local x,y,z,m00,m01,m02,
m10,m11,m12,m20,m21,m22=cf:components()local c00,c01,c02=(m12*m20)-(m10*m22),(
m00*m22)-(m02*m20),(m02*m10)-(m00*m12)local d00,d01,d02=(m10*c02)-(c01*m20),(c00
*m20)-(c02*m00),(m00*c01)-(c00*m10)local a=math.max(((m00^2)+(m10^2)+(m20^2))^
0.5,0.00001)local b=math.max(((c00^2)+(c01^2)+(c02^2))^0.5,0.00001)local c=math.
max(((d00^2)+(d01^2)+(d02^2))^0.5,0.00001)return CFrame.new(x,y,z,m00/a,c00/b,
d00/c,m10/a,c01/b,d01/c,m20/a,c02/b,d02/c)end local function sortedKeys(map,
descending)local keys={}for key in pairs(map)do table.insert(keys,key)end table.
sort(keys,function(a,b)if descending then return a>b end return a<b end)return
keys end local function buildNativeAnimationRig(character)local root=character
and character:FindFirstChild('HumanoidRootPart')if root==nil then return nil end
local motors={}for _,descendant in ipairs(character:GetDescendants())do if
descendant:IsA('Motor6D')or descendant:IsA('Motor')then table.insert(motors,
...
joints=connectedParts(item)for index,part in ipairs(parts)do local motor=joints[
index]local child={part=part,name=part.Name,motor=motor,originC1=
repairedAnimationCFrame(motor.C1),children={},parent=item,lineNumber=lineNumber}
lineNumber=lineNumber+1 table.insert(item.children,child)visit(child)end end
local rootItem={part=root,name=root.Name,motor=nil,originC1=CFrame.new(),
children={},parent=nil,lineNumber=0}visit(rootItem)return rig end local function 
keyframeTimeClamp(clip,value)if value>clip.length then return clip.length end if
clip.snapping then value=value+0.025 return value-(value%0.05)end return value
end local function getClosestPose(clip,timePosition,part)local found=nil for _,
key in ipairs(clip.sortedTimesAsc)do if key>timePosition then break end local
frame=clip.keyframes[key]if frame and frame.poses[part]then found=frame.poses[
part]end end return found end local function getClosestNextPose(clip,
timePosition,part)for _,key in ipairs(clip.sortedTimesAsc)do if key>timePosition
then local frame=clip.keyframes[key]if frame and frame.poses[part]then return
frame.poses[part]end end end return nil end local function getNativeMotorC1(clip
,item,timePosition)timePosition=keyframeTimeClamp(clip,timePosition)if item.
motor==nil or item.part==nil then return item.originC1 end local previous=
getClosestPose(clip,timePosition,item.part)if previous==nil then return item.
originC1 end local following=getClosestNextPose(clip,timePosition,item.part)if
clip.interpolate and following and previous.cframe~=following.cframe and
timePosition~=previous.time then local span=following.time-previous.time local
rawAlpha=span>0 and((timePosition-previous.time)/span)or 1 local alpha=
nativeAnimationEase(previous.easingStyle,previous.easingDirection,1-rawAlpha)
local ok,value=pcall(function()return previous.cframe:Inverse():Lerp(following.
cframe:Inverse(),alpha):Inverse()*item.originC1 end)return ok and
repairedAnimationCFrame(value)or(previous.cframe*item.originC1)end return
previous.cframe*item.originC1 end local function buildNativeAnimationClip(
sequence,rig)local clip={sequence=sequence,length=0,keyframes={},sortedTimesAsc=
{},looped=sequence.Loop==true,priority=sequence.Priority and sequence.Priority.
Name or'Core',animationFramerate=1/60,snapping=true,interpolate=true}for _,child
in ipairs(sequence:GetChildren())do if child:IsA('Keyframe')and child.Time>clip.
length then clip.length=child.Time end end clip.length=math.max(clip.length,0.05
)local function createKeyframe(timePosition)timePosition=keyframeTimeClamp(clip,
timePosition)local frame=clip.keyframes[timePosition]if frame==nil then frame={
time=timePosition,poses={},name='Keyframe'}clip.keyframes[timePosition]=frame
end return frame end local function loadPose(frame,pose)local item=rig.
partListByName[pose.Name]local weight=tonumber(pose.Weight)or 1 if item~=nil and
weight>0 then local stored={item=item,time=frame.time,easingStyle=pose.
EasingStyle and pose.EasingStyle.Name or'Linear',easingDirection=pose.
EasingDirection and pose.EasingDirection.Name or'Out'}if item.originC1~=nil then
stored.cframe=item.originC1*pose.CFrame:Inverse()*item.originC1:Inverse()else
stored.cframe=pose.CFrame end frame.poses[item.part]=stored end for _,child in
ipairs(pose:GetChildren())do if child:IsA('Pose')then loadPose(frame,child)end
end end for _,child in ipairs(sequence:GetChildren())do if child:IsA('Keyframe')
...
HumanoidRigType.R15)end)if modelOk then return fallbackModel end return nil end
local function setProxyVisualHidden(handle,hidden)if handle==nil or handle.model
==nil then return end local transparency=hidden and 1 or 0.08 handle.model.
Transparency=transparency if handle.proxyParts then for _,part in ipairs(handle.
proxyParts)do if part then part.Transparency=hidden and 1 or 0.1 end end end if
handle.label then handle.label.Visible=not hidden end end local function 
applyAppearanceAvatar(handle,anchorPart,components,entityId)if handle==nil or
anchorPart==nil or type(components)~='table'then return end local userId=
appearanceUserIdFor(components)if not userId then clearAppearanceAvatar(handle)
setProxyVisualHidden(handle,false)return end if handle.appearanceModel and
handle.appearanceUserId==userId and handle.appearanceAnchor==anchorPart then
return end if handle.appearanceLoadingId==userId then return end
clearAppearanceAvatar(handle)handle.appearanceLoadingId=userId task.spawn(
function()local model=createAppearanceModelFromUserId(userId)if not model then
if handle.appearanceLoadingId==userId then handle.appearanceLoadingId=nil
emitBridgeError('avatar.appearance_failed','Could not load avatar appearance: '
..tostring(userId))end return end if handle.appearanceLoadingId~=userId then
model:Destroy()return end prepareAppearanceModel(model)local rootPart=
appearanceRootPart(model)if not rootPart then model:Destroy()handle.
appearanceLoadingId=nil return end model.Name='OverlayAppearance_'..tostring(
entityId or userId)model.PrimaryPart=rootPart model.Parent=handle.folder or
entityFolder rootPart.CFrame=anchorPart.CFrame local weld=Instance.new(
...
name,size,offset,color,transparency)local part=Instance.new('Part')part.Name=
name part.Size=size configureProxyPart(part,color)part.Transparency=transparency
or 0 weldProxyPart(root,part,offset)return part end local function 
createProxyModel(entityId,components)local color=colorFromComponents(entityId,
components)local root=Instance.new('Part')root.Name=entityId root.Size=Vector3.
new(1.35,1.9,0.7)configureProxyPart(root,color)root.Anchored=true root.Material=
Enum.Material.Neon root.Transparency=0.08 if type(components)=='table'and type(
components.transform)=='table'then root.CFrame=transformToCFrame(components.
transform)else root.CFrame=CFrame.new(0,5,0)end root.Parent=entityFolder local
proxyParts={createProxyLimb(root,'Head',Vector3.new(0.88,0.88,0.88),CFrame.new(0
,1.38,0),color,0),createProxyLimb(root,'LeftArm',Vector3.new(0.34,1.45,0.42),
CFrame.new(-0.93,0.05,0),color,0.1),createProxyLimb(root,'RightArm',Vector3.new(
0.34,1.45,0.42),CFrame.new(0.93,0.05,0),color,0.1),createProxyLimb(root,
'LeftLeg',Vector3.new(0.42,1.25,0.46),CFrame.new(-0.36,-1.55,0),color,0.1),
createProxyLimb(root,'RightLeg',Vector3.new(0.42,1.25,0.46),CFrame.new(0.36,-
1.55,0),color,0.1)}local _,label=createBillboard(root,entityId,components,
Vector3.new(0,2.45,0))return root,label,proxyParts end local function 
applyEffectToPart(handle,parentPart,settings)if handle==nil or parentPart==nil
then return end if handle.auraAttachment==nil or handle.auraAttachment.Parent~=
parentPart then if handle.auraAttachment then handle.auraAttachment:Destroy()end
local attachment=Instance.new('Attachment')attachment.Name=
'OverlayAuraAttachment'attachment.Parent=parentPart local particle=Instance.new(
'ParticleEmitter')particle.Name='OverlayAuraParticles'particle.Texture=
'rbxasset://textures/particles/sparkles_main.dds'particle.Lifetime=NumberRange.
new(0.55,1.2)particle.Speed=NumberRange.new(0.15,0.85)particle.SpreadAngle=
Vector2.new(360,360)particle.LightEmission=0.72 particle.Parent=attachment
handle.auraAttachment=attachment handle.auraParticle=particle end if handle.
auraParticle then handle.auraParticle.Enabled=settings.enabled handle.
auraParticle.Rate=settings.enabled and math.floor(4+settings.alpha*34)or 0
handle.auraParticle.Color=ColorSequence.new(settings.color)handle.auraParticle.
Size=NumberSequence.new(0.12+settings.alpha*0.22)handle.auraParticle.
Transparency=NumberSequence.new(math.clamp(0.72-settings.alpha*0.5,0.15,0.9))end
if settings.enabled and settings.trail then if handle.trailAttachment0==nil or
handle.trailAttachment0.Parent~=parentPart then if handle.trailAttachment0 then
handle.trailAttachment0:Destroy()end if handle.trailAttachment1 then handle.
trailAttachment1:Destroy()end local a0=Instance.new('Attachment')a0.Name=
'OverlayTrailA'a0.Position=Vector3.new(-0.55,0,0)a0.Parent=parentPart local a1=
Instance.new('Attachment')a1.Name='OverlayTrailB'a1.Position=Vector3.new(0.55,0,
0)a1.Parent=parentPart handle.trailAttachment0=a0 handle.trailAttachment1=a1 end
if handle.trail==nil then local trail=Instance.new('Trail')trail.Name=
'OverlayAuraTrail'trail.Attachment0=handle.trailAttachment0 trail.Attachment1=
handle.trailAttachment1 trail.FaceCamera=true trail.MinLength=0.05 trail.Parent=
parentPart handle.trail=trail end handle.trail.Enabled=true handle.trail.Color=
ColorSequence.new(settings.color)handle.trail.Lifetime=0.18+settings.alpha*0.44
handle.trail.Transparency=NumberSequence.new(math.clamp(0.82-settings.alpha*0.48
,0.25,0.95))elseif handle.trail then handle.trail.Enabled=false end end function
ProxyAvatarRenderer.onCreate(entity)local model,label,proxyParts=
createProxyModel(entity.entityId,entity.components)entity.handle={model=model,
label=label,proxyParts=proxyParts}applyProxyAssetVisuals(entity.handle,model,
entity.components)applyAppearanceAvatar(entity.handle,model,entity.components,
entity.entityId)applyDescriptorMorph(entity.handle,model,entity.components,
entity.entityId,model)applyEffectToPart(entity.handle,model,effectSettings(
entity.entityId,entity.components))end function ProxyAvatarRenderer.onPatch(
entity,patch)local handle=entity.handle if handle==nil then return end local
color=colorFromComponents(entity.entityId,entity.components)if handle.label then
handle.label.Text=displayNameFor(entity.entityId,entity.components)handle.label.
TextColor3=color end if handle.model then handle.model.Color=color if handle.
proxyParts then for _,part in ipairs(handle.proxyParts)do if part then part.
Color=color end end end if type(patch.transform)=='table'then handle.model.
CFrame=transformToCFrame(patch.transform)entity.samples={}end
...
applyEffectToPart(handle,handle.model,effectSettings(entity.entityId,entity.
components))end end function ProxyAvatarRenderer.onDestroy(entity)destroyHandle(
entity.handle)entity.handle=nil end function ProxyAvatarRenderer.
onEphemeralTransform(entity,transform)pushSample(entity,transform)end local 
function clearNativeAttachedVisuals(handle)for _,key in ipairs({'billboard',
'highlight','auraParticle','auraAttachment','trail','trailAttachment0',
'trailAttachment1'})do if handle[key]then pcall(function()handle[key]:Destroy()
end)handle[key]=nil end end end function OwnCharacterOverlayRenderer.onCreate(
entity)local folder=Instance.new('Folder')folder.Name=entity.entityId..
'_self_overlay'folder.Parent=entityFolder entity.handle={folder=folder,
connections={LocalPlayer.CharacterAdded:Connect(function()task.wait(0.1)local
current=Client.entities[entity.entityId]if current==entity and current.
rendererKind=='self'then OwnCharacterOverlayRenderer.onPatch(entity,{})end end)}
}OwnCharacterOverlayRenderer.onPatch(entity,{})end function
OwnCharacterOverlayRenderer.onPatch(entity)local handle=entity.handle if handle
==nil then return end local character=LocalPlayer.Character if character==nil
then return end local head=character:FindFirstChild('Head')local root=character:
FindFirstChild('HumanoidRootPart')local anchor=head or root if anchor==nil then
return end local morphAssetId=assetIdFromComponent(entity.components and entity.
components.morph)local morphDescriptor=morphAssetId and descriptorForAsset(
morphAssetId)or nil local morphAnchor=resolveMorphAnchor(character,
morphDescriptor,anchor)if root then applyAppearanceAvatar(handle,root,entity.
components,entity.entityId)applyEffectToPart(handle,root,effectSettings(entity.
entityId,entity.components))else applyAppearanceAvatar(handle,anchor,entity.
components,entity.entityId)end applyDescriptorMorph(handle,morphAnchor,entity.
components,entity.entityId,character)applyAnimationToCharacter(handle,character,
entity.components,entity.entityId)end function OwnCharacterOverlayRenderer.
onDestroy(entity)destroyHandle(entity.handle)entity.handle=nil end function
OwnCharacterOverlayRenderer.onEphemeralTransform()end function
NativeCharacterOverlayRenderer.onCreate(entity)local folder=Instance.new(
'Folder')folder.Name=entity.entityId..'_native_overlay'folder.Parent=
entityFolder entity.handle={folder=folder,connections={}}
NativeCharacterOverlayRenderer.onPatch(entity,{})end function
NativeCharacterOverlayRenderer.onPatch(entity)local handle=entity.handle if
handle==nil then return end local player=findPlayerByRobloxUserId(
robloxUserIdFor(entity.components))if player==nil or player==LocalPlayer then
return end if handle.player~=player then for _,connection in ipairs(handle.
connections or{})do pcall(function()connection:Disconnect()end)end handle.
connections={player.CharacterAdded:Connect(function()task.wait(0.1)local current
=Client.entities[entity.entityId]if current==entity and current.rendererKind==
'native'then NativeCharacterOverlayRenderer.onPatch(entity,{})end end)}handle.
player=player clearNativeAttachedVisuals(handle)end local character=player.
Character if character==nil then return end local head=character:FindFirstChild(
'Head')local root=character:FindFirstChild('HumanoidRootPart')local anchor=head
or root if anchor==nil then return end local morphAssetId=assetIdFromComponent(
entity.components and entity.components.morph)local morphDescriptor=morphAssetId
and descriptorForAsset(morphAssetId)or nil local morphAnchor=resolveMorphAnchor(
character,morphDescriptor,anchor)local color=colorFromComponents(entity.entityId
,entity.components)local effect=effectSettings(entity.entityId,entity.components
)if handle.billboard==nil or handle.billboard.Parent~=anchor then if handle.
billboard then handle.billboard:Destroy()end handle.billboard,handle.label=
createBillboard(anchor,entity.entityId,entity.components,Vector3.new(0,2.8,0))
end if handle.label then handle.label.Text=displayNameFor(entity.entityId,entity
.components)handle.label.TextColor3=color end if handle.highlight==nil and
activeOverlayHighlightCount()>=CONFIG.max_native_highlights then elseif handle.
highlight==nil or handle.highlight.Adornee~=character then if handle.highlight
then handle.highlight:Destroy()end local highlight=Instance.new('Highlight')
highlight.Name='OverlayAura'highlight.Adornee=character highlight.
FillTransparency=0.86 highlight.OutlineTransparency=0.12 pcall(function()
highlight.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop end)highlight.Parent=
handle.folder handle.highlight=highlight end if handle.highlight then handle.
highlight.FillColor=color handle.highlight.OutlineColor=color handle.highlight.
FillTransparency=math.clamp(0.95-effect.alpha*0.22,0.68,0.98)handle.highlight.
OutlineTransparency=math.clamp(0.45-effect.alpha*0.3,0.08,0.75)end if root then
applyAppearanceAvatar(handle,root,entity.components,entity.entityId)
applyEffectToPart(handle,root,effect)else applyAppearanceAvatar(handle,anchor,
entity.components,entity.entityId)end applyDescriptorMorph(handle,morphAnchor,
entity.components,entity.entityId,character)applyAnimationToCharacter(handle,
character,entity.components,entity.entityId)end function
NativeCharacterOverlayRenderer.onDestroy(entity)destroyHandle(entity.handle)
entity.handle=nil end function NativeCharacterOverlayRenderer.
onEphemeralTransform()end local function ensureRenderer(entityId)local entity=
Client.entities[entityId]if entity==nil then return end local desired=
selectRendererKind(entityId,entity.components)if entity.rendererKind~=desired
then local oldRenderer=entity.rendererKind and Renderers[entity.rendererKind]if
oldRenderer then oldRenderer.onDestroy(entity)else destroyHandle(entity.handle)
entity.handle=nil end entity.rendererKind=desired entity.samples={}if desired
and Renderers[desired]then Renderers[desired].onCreate(entity)end elseif desired
and Renderers[desired]then Renderers[desired].onPatch(entity,{})end end local 
function refreshRenderers()for entityId in pairs(Client.entities)do
ensureRenderer(entityId)end end local function scheduleRendererRefresh()task.
spawn(function()task.wait(0.1)if Client.running then refreshRenderers()end end)
end trackConnection(Players.PlayerAdded:Connect(function(player)trackConnection(
player.CharacterAdded:Connect(scheduleRendererRefresh))scheduleRendererRefresh()
end))trackConnection(Players.PlayerRemoving:Connect(scheduleRendererRefresh))for
_,player in ipairs(Players:GetPlayers())do trackConnection(player.CharacterAdded
:Connect(scheduleRendererRefresh))end local function upsertEntity(entityId,
components)prefetchAssetRefs(components)local entity=Client.entities[entityId]if
entity==nil then entity={entityId=entityId,components=components,samples={}}
Client.entities[entityId]=entity else entity.components=components end
ensureRenderer(entityId)end local function patchEntity(entityId,patch)
prefetchAssetRefs(patch)local entity=Client.entities[entityId]if entity==nil
then upsertEntity(entityId,patch)return end entity.components=entity.components
or{}for key,value in pairs(patch)do entity.components[key]=value end
ensureRenderer(entityId)if entity.rendererKind and Renderers[entity.rendererKind
]then Renderers[entity.rendererKind].onPatch(entity,patch)end end local function 
destroyEntity(entityId)local entity=Client.entities[entityId]if entity==nil then
return end if entity.rendererKind and Renderers[entity.rendererKind]then
Renderers[entity.rendererKind].onDestroy(entity)else destroyHandle(entity.handle
)end Client.entities[entityId]=nil end local function clearEntities()for
entityId in pairs(Client.entities)do destroyEntity(entityId)end Client.entities=
{}end local function clearCurrentRoom(roomId)if Client.room and(roomId==nil or
Client.room.id==roomId)then Client.room=nil end if roomId==nil or Client.
...
cf elseif following==nil then handle.model.CFrame=previous.cf else local span=
following.at-previous.at local alpha=span>0 and math.clamp((renderAt-previous.at
)/span,0,1)or 1 handle.model.CFrame=previous.cf:Lerp(following.cf,alpha)end end
if handle and handle.descriptorMorphJiggle then for _,item in ipairs(handle.
descriptorMorphJiggle)do if item.bone and item.bone.Parent then local sway=math.
sin(now*item.speed+item.phase)*item.amplitude local twist=math.cos(now*(item.
speed*0.7)+item.phase)*item.amplitude*0.45 safeSet(item.bone,'Transform',item.
base*CFrame.Angles(sway,0,twist))end end end if handle and handle.
nativeMorphSmartBones then local smartBoneStartedAt=os.clock()local
smartBoneBudget=tonumber(CONFIG.native_morph_smart_bone_frame_budget_seconds)or
0 for _,controller in ipairs(handle.nativeMorphSmartBones)do if not controller.
disabled then local ok=pcall(updateSmartBoneController,controller,delta)if not
ok then controller.disabled=true end end if smartBoneBudget>0 and os.clock()-
smartBoneStartedAt>=smartBoneBudget then break end end end end end))local 
function countKeys(map)local count=0 for _ in pairs(map or{})do count=count+1
end return count end local function applySnapshot(data)if type(data)~='table'
then return end if Client.room==nil or data.room_id~=Client.room.id then Client.
deferredSnapshot=data return end Client.deferredSnapshot=nil AssetCache.
registerAssets(data.assets or{})clearEntities()Client.room.version=tonumber(data
.version)or Client.room.version or 0 Client.room.ownerUserId=data.owner_user_id
for entityId,entity in pairs(data.entities or{})do upsertEntity(entityId,entity.
components or{})end log(('snapshot applied: version=%d entities=%d'):format(
Client.room.version,countKeys(data.entities)))emitBridgeEvent('room.snapshot',
data)end local function resync()if Client.room==nil or Client.room.id==nil then
return end log('delta gap detected; requesting fresh snapshot')Client.joining={
roomId=Client.room.id,resumeFrom=0}sendMessage('room.join',mergeTables({room_id=
Client.room.id,resume_from_version=0},currentRobloxJoinContext()))end local 
function applyDelta(data)if type(data)~='table'then return end if Client.room==
nil or data.room_id~=Client.room.id then return end if data.base_version~=Client
.room.version then resync()return end Client.room.version=data.version for _,op
in ipairs(data.ops or{})do if op.op=='entity.create'then upsertEntity(op.
entity_id,op.components or{})elseif op.op=='component.patch'then patchEntity(op.
entity_id,op.components or{})elseif op.op=='entity.destroy'then destroyEntity(op
.entity_id)end end end local function applyRoomState(data)if type(data)~='table'
then return end if Client.room==nil or data.room_id~=Client.room.id then return
end for _,state in ipairs(data.states or{})do if state.entity_id~=Client.
ownAvatarId then local entity=Client.entities[state.entity_id]local renderer=
entity and entity.rendererKind and Renderers[entity.rendererKind]if renderer and
type(state.transform)=='table'then renderer.onEphemeralTransform(entity,state.
transform)end end end end local function handleServerMessage(raw)local ok,
message=pcall(HttpService.JSONDecode,HttpService,raw)if not ok or type(message)
~='table'then return end if type(message.data)~='table'then message.data={}end
Client.lastServerContact=os.clock()if tonumber(message.data.server_time_ms)then
Client.serverTimeOffsetMs=tonumber(message.data.server_time_ms)-(os.clock()*1000
)end if message.t=='room.joined'and type(message.data)=='table'then Client.room=
Client.room or{}Client.room.id=message.data.room_id Client.room.ownerUserId=
message.data.owner_user_id local resumeFrom=0 if Client.joining and Client.
joining.roomId==message.data.room_id then resumeFrom=Client.joining.resumeFrom
or 0 end if resumeFrom>0 then Client.room.version=Client.room.version or
resumeFrom else Client.room.version=message.data.current_version or Client.room.
version or 0 end if Client.deferredSnapshot and Client.deferredSnapshot.room_id
==Client.room.id then applySnapshot(Client.deferredSnapshot)end emitBridgeEvent(
'room.join',{room_id=Client.room.id,current_version=Client.room.version,
roblox_route=message.data.roblox_route})Client.joining=nil end if message.t==
'room.deleted'and type(message.data)=='table'then if Client.room and Client.room
.id==message.data.room_id then clearCurrentRoom(message.data.room_id)end
emitBridgeEvent('room.deleted',message.data)end if message.t=='room.kicked'and
type(message.data)=='table'then if Client.room and Client.room.id==message.data.
room_id then clearCurrentRoom(message.data.room_id)end emitBridgeEvent(
'room.kicked',message.data)end local requestId=message.data and message.data.
request_id if requestId and Client.pending[requestId]then Client.pending[
requestId].callback(message)return end if message.t=='room.snapshot'then
applySnapshot(message.data)elseif message.t=='room.delta'then applyDelta(message
.data)elseif message.t=='room.state'then applyRoomState(message.data)elseif
message.t=='room.member.joined'then log('member joined:',message.data.
display_name or message.data.user_id)emitBridgeEvent('room.member.joined',
message.data)elseif message.t=='room.member.left'then log('member left:',message
.data.user_id)emitBridgeEvent('room.member.left',message.data)elseif message.t==
'room.invited'then emitBridgeEvent('room.invited',message.data)elseif message.t
=='chat.message'then log(('[chat][%s] %s: %s'):format(tostring(message.data.
...
nativeMorphCatalog if type(assets)~='table'or#assets==0 then assets=Client.
assetCatalog end return{connected=Client.connected,user=Client.user,room=Client.
room,rooms=Client.cachedRooms,assets=assets,animations=Client.
nativeAnimationCatalog,asset_slots=Client.nativeMorphCatalogSlots,
...
return end if not Client.connected then emitBridgeError('runtime.disconnected',
'Runtime is not connected')return end task.spawn(function()local response,err=
awaitRequest('room.kick',{room_id=roomId,user_id=userId})if response==nil then
emitBridgeError('room.kick_failed',tostring(err))return end emitBridgeState(
'kicked '..tostring(userId))end)end local function bridgeSendChat(scope,text,
roomId)text=trimString(text)if text==''then emitBridgeError('chat.empty',
'Message is empty')return end if not Client.connected then emitBridgeError(
'runtime.disconnected','Runtime is not connected')return end local outgoing={
scope=scope=='global'and'global'or'room',text=text}if outgoing.scope=='room'then
outgoing.room_id=trimString(roomId)~=''and trimString(roomId)or(Client.room and
Client.room.id)if not outgoing.room_id then emitBridgeError('room.required',
'Join a room before sending room chat')return end end sendMessage('chat.send',
outgoing)end local function isHttpUrl(value)value=string.lower(trimString(value)
)return string.sub(value,1,7)=='http://'or string.sub(value,1,8)=='https://'end
local function assetTypeForBridge(kind,url)local cleanUrl=string.lower(tostring(
url or''))if string.find(cleanUrl,'%.png',1,false)or string.find(cleanUrl,
'%.jpg',1,false)or string.find(cleanUrl,'%.jpeg',1,false)or string.find(cleanUrl
,'%.webp',1,false)then return'texture'end if string.find(cleanUrl,'%.mesh',1,
false)then return'mesh'end if kind=='animation'then return'animation'end return
kind..'_bundle'end local function assetFormatForBridge(kind,url)local cleanUrl=
string.lower(tostring(url or''))if string.find(cleanUrl,'%.json',1,false)then
return'overlay_json_v1'end if string.find(cleanUrl,'%.rbxmx',1,false)then return
'rbxmx'end return nil end local function assetIdFromUrl(kind,url)local cleanUrl=
string.match(tostring(url or''),'^[^%?]+')or tostring(url or'')local fileName=
string.match(cleanUrl,'([^/\\]+)$')or cleanUrl fileName=string.gsub(fileName,
'%.[%w]+$','')local assetId=safeFileName(kind..'_'..fileName)if assetId==''or
...
payload.source)~=''and trimString(payload.source)or'runtime-ui'}local size=
tonumber(payload.size)if size then asset.size=math.floor(size)end local response
,err=awaitRequest('asset.register',asset,CONFIG.request_timeout+5)if response==
nil then return nil,'asset.register failed: '..tostring(err)end local registered
=response.data and response.data.asset or asset AssetCache.registerAssets({
registered})emitBridgeEvent('asset.registered',{asset=registered})return
registered end if assetId==''then return nil end local catalogAsset=Client.
assetCatalogById[assetId]if catalogAsset and type(catalogAsset.url)=='string'and
catalogAsset.url~=''then return resolveBridgeAsset(kind,catalogAsset)end local
response,err=awaitRequest('asset.get',{asset_id=assetId},CONFIG.request_timeout)
if response==nil then return nil,'asset.get failed: '..tostring(err)end local
asset=response.data and response.data.asset if asset==nil then return nil,
'asset.get returned no asset'end AssetCache.registerAssets({asset})return asset
end local function bridgeRequireRoom(action)if not Client.connected then
emitBridgeError('runtime.disconnected','Runtime is not connected')return nil end
if not Client.room or not Client.room.id then emitBridgeError('room.required',
'Join a room before '..action)return nil end return Client.room.id end local 
...
if Client.sessionId~=sessionId or Client.ws~=ws then return end
handleServerMessage(raw)end))trackWsConnection(ws.OnClose:Connect(function()if
Client.sessionId~=sessionId or Client.ws~=ws then return end Client.connected=
false end))local helloResponse,helloError=awaitRequest('hello',{client=CONFIG.
client_name,build=CONFIG.build})if helloResponse==nil then return false,
'hello failed: '..tostring(helloError)end Client.heartbeatSeconds=math.max((
helloResponse.data.heartbeat_ms or 15000)/1000,5)local token=CONFIG.token or(
'dev:'..LocalPlayer.Name)local authResponse,authError=awaitRequest('auth',{token
=token})if authResponse==nil then return false,'auth failed: '..tostring(
authError)end Client.user=authResponse.data Client.ownAvatarId='avatar_'..Client
...
function OverlayUI:_animationLabel(animationOrId)
	local animation = animationOrId
	if type(animationOrId) == "string" then
...
function OverlayUI:_selectedAnimationName()
	local selected = self.State.SelectedAnimation
	if type(selected) == "string" and self.AnimationById[selected] then
		local animation = self.AnimationById[selected]
		return tostring(animation.display_name or animation.name or selected)
	end
	if type(selected) == "string" and selected ~= PLACEHOLDER_ANIMATION then
		return selected
	end
...
function OverlayUI:_refreshAnimationDropdown()
	local dropdown = self.Controls.AnimationDropdown
	if not dropdown then
		return
	end

	local values = self:_animationValues()
	local selected = self.State.SelectedAnimation
	local selectedValue = nil
	if type(selected) == "string" then
		if self.AnimationById[selected] then
	...
	self.RefreshingAnimations = true
	if dropdown.SetValues then
		dropdown:SetValues(values)
	end
	if dropdown.SetDisabledValues then
		dropdown:SetDisabledValues(#self.State.Animations == 0 and {} or {})
	end
	if dropdown.SetValue then
		dropdown:SetValue(selectedValue)
	end
	self.State.SelectedAnimation = selectedValue
	self.RefreshingAnimations = false
...
function OverlayUI:_buildAnimation()
	...
	self.Controls.AnimationDropdown = picker:AddDropdown("Overlay_AnimationPreset", {
		...
		MaxVisibleDropdownItems = 8,
		FormatDisplayValue = function(value)
			return self:_animationLabel(value)
		end,
		FormatListValue = function(value)
			return self:_animationLabel(value)
		end,
		Callback = function(value)
			if self.RefreshingAnimations then
				return
			end
...
function OverlayUI:HandleRuntimeEvent(eventName, payload)
	...
	elseif eventName == "animation.catalog" then
...
if type(GLOBAL.OverlayBridge) ~= "table" then
	log("runtime bridge was not ready; opening UI anyway")
end
